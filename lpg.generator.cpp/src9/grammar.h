...
private:
    ...
    Array< Tuple<int> > special_nonterminal_array;
    ...
    int GetProsthesisNameIndex(int, int, int);
    int GetClassnameIndex(int, int, int, int);
...
public:
    ...
    void ProcessInput(Action *);
    void ProcessActions(Action *, Base *, Produce *);
    void ProcessGrammarOutput(Action *);
    ...
    Tuple<int> rhs_sym;
    Tuple<int> keywords;
    Tuple<int> prosthesis_rule_index;
    Tuple<char *> name;
    ...
    Tuple<int> &RulesOfLhs(int nt)
    {
        return lhs_rule[nt];
    }

    int RuleNumberOf(int rule_no)
    {
        int lhs_symbol = rules[rule_no].lhs;
        for (int i = 0; i < lhs_rule[lhs_symbol].Length(); i++)
            if (lhs_rule[lhs_symbol][i] = rule_no) return i + 1;
        assert(0);
        return 0; // we will never get here ... but it avoids a warning
    }
    ...
    const char *RetrieveName(int i)   { return (name.Length() == 0 ? "" : name[symbol_index[i].external_name_index]); }
    ...
    void RestoreSymbol(TextBuffer &, char *);
    void RestoreSymbol(char *, const char *);

...
//
void Base::ProcessBasicMaps(void)
{
    //
    ...
    // the non-terminal.
    //
    nt_first.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
    ...
    //
    closure.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
    ...
    //
    //
    generates_null.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
    index_of.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
    index_of.Initialize(OMEGA);
    {
        for (int nt = grammar -> FirstNonTerminal(); nt <= grammar -> LastNonTerminal(); nt++)
    ...
    nt_items.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
    ...
    item_no = first_item_of[grammar -> RulesOfLhs(grammar -> accept_image)[0]];
    ...
    //
    clitems.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
    for (int nt = grammar -> FirstNonTerminal(); nt <= grammar -> LastNonTerminal(); nt++)
    {
        assert(clitems[nt].Length() == 0);
        Tuple<int> &rules = grammar -> RulesOfLhs(nt);
        for (int i = 0; i < rules.Length(); i++)
            clitems[nt].Next() = first_item_of[rules[i]];
    }
    ...
    //
    {
        Tuple<int> bad_list,
                   depend_list;
        generates_string.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
        is_cyclic.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
        index_of.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
        index_of.Initialize(OMEGA);
        for (int nt = grammar -> FirstNonTerminal(); nt <= grammar -> LastNonTerminal(); nt++)
        {
        ...
        if (bad_list.Length() > 0 || depend_list.Length())
        {
            for (int i = 0; i < bad_list.Length(); i++)
            {
                ...
                TextBuffer msg;
                msg.Put("The nonterminal ");
                msg.Put(tok);
                msg.Put(" cannot generate complete strings of terminals");
            ...
            for (int i = 0; i < depend_list.Length(); i++)
            {
                ...
                msg.Put("The nonterminal ");
                msg.Put(tok);
                msg.Put(" depends on nonterminals that cannot generate complete strings of terminals");
                option -> EmitError(grammar -> RetrieveTokenLocation(symbol), msg);
            }
            */
    ...
    {
        string_generated.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
        prosthesis_rule.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
        index_of.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
        index_of.Initialize(OMEGA);
        {
            for (int nt = grammar -> FirstNonTerminal(); nt <= grammar -> LastNonTerminal(); nt++)
    ...
    if (option -> lalr_level > 1)
    {
        direct_produces.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
        produces.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
        {
            for (int nt = grammar -> FirstNonTerminal(); nt <= grammar -> LastNonTerminal(); nt++)
    ...
    //
    if (option -> slr || option -> follow || option -> error_maps || grammar -> RulesOfLhs(grammar -> accept_image).Length() > 1)
    {
        follow.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
        for (int i = grammar -> FirstNonTerminal(); i <= grammar -> LastNonTerminal(); i++)
        ...
        //
        if (grammar -> RulesOfLhs(grammar -> accept_image).Length() > 1)
        {
            int rule_no = grammar -> RulesOfLhs(grammar -> accept_image)[0],
                top = item_table[first_item_of[rule_no]].suffix_index;
            for (int i = top + 1, k = 0; i < first.Length(); i++, k++)
            {
                int rule_no = grammar -> RulesOfLhs(grammar -> accept_image)[k],
    ...
    if (option -> backtrack)
    {
        ...
        lhs_ranked_rules.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
            ...
            for (int i = 0; i < grammar -> check_predecessor_sets_for.Length(); i++)
            {
                ...
                for (int k = 0; k < inter.Size(); k++)
                {
                    if (inter[k])
                    {
                        ...
                        TextBuffer msg;
                        msg.Put("This symbol is a predecessor of both \"");
                        msg.Put(left_tok);
                        msg.Put("\"  and \"");
                        msg.Put(right_tok);
                        msg.Put("\"");
...
void Base::NoRulesProduced(void)
{
    ...
    {
        if (grammar -> RulesOfLhs(symbol).Length() == 0)
            list.Next() = symbol;
    }
    ...
    if (list.Length() > 0)
    {
        for (int i = 0; i < list.Length(); i++)
        {
            ...
            msg.Put("The nonterminal ");
            msg.Put(tok);
            msg.Put(" does not produce any rule.");
            option -> EmitError(grammar -> RetrieveTokenLocation(symbol), msg);
        }
...
{
    BoundedArray<int> nont_list(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal(), OMEGA);
    ...
    Tuple<int> &rules = grammar -> RulesOfLhs(lhs_symbol);
    for (int i = 0; i < rules.Length(); i++)
    {
        int rule_no = rules[i],
...
void Base::NullablesComputation(void)
{
    ...
    while(changed)
    {
        ...
        {
            Tuple<int> &rules = grammar -> RulesOfLhs(nt);
            for (int i = 0; i < rules.Length() && (! null_nt[nt]); i++)
            {
                int rule_no = rules[i];
...
// derived from x.
//
void Base::ComputeStringGeneration(int nt)
{
    stack.Push(nt);
    int indx = stack.Length();
    index_of[nt] = indx;
    string_generated[nt].Reset();
    ...
    //
    bool successful = generates_null[nt];

    //
    // If a nonterminal is nullable, we chose the NULL string as an examplar for it.
    //
    prosthesis_rule[nt] = 0; // for nullable nonterminal, this does not matter.
    if (! successful)
    {
        Tuple<int> chosen_rhs,
                   temp_rhs;
        //
        ...
        for (int k = 0; k < rules.Length(); k++)
        {
            ...
            int rule_no = rules[k],
                i;
            for (i = grammar -> FirstRhsIndex(rule_no); i < grammar -> EndRhsIndex(rule_no); i++)
            {
                int symbol = grammar -> rhs_sym[i];
                if (grammar -> IsTerminal(symbol))
                    temp_rhs.Next() = symbol;
                else if (! generates_null[symbol])
                {
                    if (index_of[symbol] == OMEGA)
                    ...
                    // to the list being constructed for rule_no. Otherwise, move to the 
                    // next rule associated with nt, if any.
                    //
                    if (index_of[symbol] == INFINITY &&
                        (string_generated[symbol].Length() != 1 || string_generated[symbol][0] != OMEGA))
                    {
                       for (int j = 0; j < string_generated[symbol].Length(); j++)
                            temp_rhs.Next() = string_generated[symbol][j];
                    }
                    else break;
                }
            }
            if (i < grammar -> EndRhsIndex(rule_no)) // not at end of rule?
                continue;
            if (chosen_rhs.Length() == 0 || temp_rhs.Length() < chosen_rhs.Length())
            {
                successful = true; // at least one string was found
                prosthesis_rule[nt] = rule_no;

                chosen_rhs.Reset();
                for (int j = 0; j < temp_rhs.Length(); j++)
                    chosen_rhs.Next() = temp_rhs[j];
                if (chosen_rhs.Length() == 1) // we're done!
                    break;
            }
        }
        ...
        //
        if (chosen_rhs.Length() == 0)
            string_generated[nt].Next() = OMEGA;
        else
        {
            for (int j = 0; j < chosen_rhs.Length(); j++)
                string_generated[nt].Next() = chosen_rhs[j];
        }
    }
    ...
    //
    if (successful || index_of[nt] == indx)
    {
        for (int symbol = stack.Top(); symbol != nt; symbol = stack.Top())
        {
            index_of[symbol] = OMEGA; // reset index so that symbol can be retried with completed NT later
            stack.Pop();
        }
        index_of[nt] = INFINITY;
        stack.Pop();
...
void Base::CheckCanGenerateNull(int nt)
{
    ...
    //
    Tuple<int> &rules = grammar -> RulesOfLhs(nt);
    for (int k = 0; k < rules.Length() && (! generates_null[nt]); k++)
    {
        int rule_no = rules[k];
...
{
    rmpself.Resize(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
...
void Base::CheckDuplicateRules(void)
{
    ...
    for (int symbol = grammar -> FirstNonTerminal(); symbol <= grammar -> LastNonTerminal(); symbol++)
    {
        Tuple<int> &rules = grammar -> RulesOfLhs(symbol);
        for (int j = 0; j < rules.Length(); j++)
        {
            int this_rule = rules[j]; // rule for which we are checking duplication
            for (int k = 0; k < j; k++)
            {
                int rule_no = rules[k];
...
//
void Base::ProcessBaseOutput(void)
{
    PrintXref();

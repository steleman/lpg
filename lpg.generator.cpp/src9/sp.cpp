...
void Sp::ComputeSingleProductionAction(int state_no, int symbol, int action)
{
    ...
    {
        for (Node *item_ptr = dfa -> statset[action].complete_items; item_ptr != NULL; item_ptr = item_ptr -> next)
        {
...
int Sp::DefaultAction(int state_no, int rule_no)
{
    ...
    while (IsSingleProductionRule(rule_no))
    {
        ...
        else
        {
            ...
            //
            Dfa::ReduceHeader &red = dfa -> statset[action].reduce;
...
void Sp::RemoveSingleProductions(void)
{
    ...
    new_action.Resize(dfa -> num_states + 1);
    update_action.Resize(dfa -> num_states + 1);
    //
        ...
        if (go_to.Length() > 0)
        {
            ...
            for (int symbol = symbol_root; symbol != Util::NIL; symbol_list[symbol] = Util::OMEGA, symbol = symbol_root)
            {
                ...
                if (IsSingleProductionRhs(symbol))
                {
                    ...
                    {
                        Node *item_ptr = dfa -> statset[action].kernel_items;
                        ...
                        if (IsSingleProductionRule(rule_no))
                        {
                            ...
                            else
                            {
                                int sp_state_no = StateMap(rule_head,
                                                           ...
                                                           sp_action_count,
                                                           symbol);
                                action = dfa -> num_states + 1 + sp_state_no;
                            }
                            ...
                            if (option -> states && original_action < 0)
                            {
                                Node *p = node_pool -> AllocateNode();
                                p -> value = item_no - 1;
                                p -> next = dfa -> statset[state_no].single_production_items;
                                dfa -> statset[state_no].single_production_items = p;
                            }
                        }
                    }
                }
    ...
    } // for (state_no = 1; state_no <= dfa -> num_states; state_no++)
...
void Sp::AddSingleProductionStates()
{
    if (sp_state.Length() > 0)
    {
        ...
        for (int offset = 0; offset < sp_state.Length(); offset++)
        {
            ...
            int state_no = dfa -> num_states + offset + 1; // new state number
            ...
            // shift actions.
            //
            dfa -> statset[state_no].kernel_items = NULL;
            dfa -> statset[state_no].complete_items = state.complete_items;
            dfa -> statset[state_no].single_production_items = NULL;
            dfa -> statset[state_no].shift_number = 0;
            dfa -> statset[state_no].transition_symbol = grammar -> empty;

            assert(dfa -> statset[state_no].go_to.Length() == 0);
            assert(dfa -> statset[state_no].predecessors.Length() == 0);
...
void Sp::UpdateBaseAutomaton()
{
    ...
    //
    dfa -> num_states += sp_state.Length();
...
void Sp::UpdateConflictActions()
{
    ...
    Array<int> conflict_count(dfa -> conflicts.Length());
    ...
    for (int i = 0; i < candidates.Length(); i++)
    {
        ...
        for (int j = conflict_index; dfa -> conflicts[j] != 0; j++)
        {
            int index = cells.NextIndex();
            cells[index].priority = cells.Length(); // keep current order.
            if (dfa -> conflicts[j] < 0) // found the shift reduce action
            {
                int rule_no = -(dfa -> conflicts[j]),
                    lhs_symbol = grammar -> rules[rule_no].lhs;
                cells[index].action = go_to.Action(lhs_symbol);
            }
            else cells[index].action = dfa -> conflicts[j];
        }

        if (conflict_count[conflict_index] > 1)
        {
             conflict_count[conflict_index]--;
             conflict[candidates[i].conflict_index].SetConflictIndex(dfa -> MapConflict(cells));
        }
        else dfa -> RemapConflict(conflict_index, cells);
    }
...
void Sp::UpdateLookaheadAutomata()
{
    ...
    for (int la_state = dfa -> num_states + 1; la_state < pda -> max_la_state; la_state++)
    {
        ...
        {
            Dfa::GotoHeader &go_to = dfa -> statset[source_state].go_to;
            for (int i = 0; i < go_to.Length(); i++)
                index_of[go_to[i].Symbol()] = i;
            ...
            Dfa::ShiftHeader &sh = dfa -> Shift(la_state);
            for (int j = 0; j < sh.Length(); j++)
            {
            ...
            if (any_new_action)
            {
                ...
                // we reused the shift map.
                //
                int shift_no = dfa -> statset[la_state].shift_number;
                if (shift_count[shift_no] > 1)
                {
                     shift_count[shift_no]--;
                     dfa -> statset[la_state].shift_number = dfa -> UpdateShiftMaps(shift_list, shift_action);
                     shift_count[dfa -> statset[la_state].shift_number]++;
                }
                else dfa -> ResetShiftMap(shift_no, shift_list, shift_action);
            }
        }
...
void Sp::MoveLaStates()
{
    ...
    dfa -> statset.Resize(pda -> max_la_state + offset + 1);
    if (produce -> gd_index.Length() > 0) // see class PRODUCE
    {
        //
        ...
        //
        for (int i = produce -> gd_index.Length(); i <= dfa -> statset.Length(); i++)
            produce -> gd_index.Next() = produce -> gd_range.Length(); // gate !!!
    }
    ...
    //
    for (int state_num = pda -> max_la_state; state_num > dfa -> num_states; state_num--)
    {
        Dfa::StateElement &new_state = dfa -> statset[state_num + offset],
                          &old_state = dfa -> statset[state_num];
        ...
        if (old_state.predecessors.Length() == 1)
        {
            ...
            new_state.predecessors.Next() = (state_no > dfa -> num_states ? state_no + offset
                                                                          : state_no);
        }
    ...
    //
    for (int k = 1; k <= dfa -> num_shift_maps; k++)
    {
        Dfa::ShiftHeader &sh = dfa -> shift[k];

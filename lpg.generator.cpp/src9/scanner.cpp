...
void Scanner::ReportErrors()
{
    ...
    for (int i = 0; i < warning_tokens.Length(); i++)
    {
        ...
        {
            case LEGACY_KEYWORD:
                 msg.Put("The use of the escaped symbol \"");
                 msg.Put(escape);
                 msg.Put(name + 1); // lex_stream -> NameString(token_index) + 1;
                 msg.Put("\" as a keyword has been deprecated. The proper spelling is %");
                 msg.Put(name + 1); // lex_stream -> NameString(token_index) + 1;
                 break;
            case SYMBOL_WITH_KEYWORD_MARKER:
                 msg.Put("The symbol \"");
                 msg.Put(name); // lex_stream -> NameString(token_index);
                 msg.Put("\" starts with the keyword marker %. It should be quoted.");
                 break;
            case INCLUDE_OPTIONS:
                 msg.Put("Some options may have been imported from the included file \"");
                 msg.Put(name);
                 msg.Put("\"");
                 break;
            default:
    ...
    if (error_tokens.Length() > 0)
    {
        for (int i = 0; i < error_tokens.Length(); i++)
        {
            ...
            {
                case NO_INPUT:
                     msg.Put("No input file specified");
                     break;
                case NO_TEMPLATE:
                     msg.Put("The template file ");
                     msg.Put(option -> template_name);
                     msg.Put(" could not be read.");
                     break;
                case NO_INCLUDE:
                     msg.Put("The include file ");
                     msg.Put(name); // lex_stream -> NameString(token_index);
                     msg.Put(" could not be read.");
                     break;
                case RECURSIVE_INCLUDE:
                     msg.Put("Attempt to recursively include file ");
                     msg.Put(name); // lex_stream -> NameString(token_index);
                     break;
                case BAD_UNICODE:
                     msg.Put("Symbol \"");
                     msg.Put(name); // lex_stream -> NameString(token_index);
                     msg.Put("\" contains an invalid unicode character.");
                     break;
                case BAD_OCTAL_ASCII_CODE:
                     msg.Put("Symbol \"");
                     msg.Put(name); // lex_stream -> NameString(token_index);
                     msg.Put("\" contains an invalid ASCII octal code.");
                     break;
                case ISOLATED_BACKSLASH:
                     msg.Put("Symbol \"");
                     msg.Put(name); // lex_stream -> NameString(token_index);
                     msg.Put("\" contains an isolated backslash.");
                     break;
                case UNDELIMITED_STRING_SYMBOL:
                     msg.Put("Quoted symbol not properly delimited. The closing quote must be followed by a space");
                     break;
                case UNTERMINATED_STRING_SYMBOL:
                     msg.Put("Quoted symbol not properly terminated");
                     break;
                case UNTERMINATED_BRACKET_SYMBOL:
                     msg.Put("Symbol enclosed in angular brackets not properly terminated");
                     break;
                case UNTERMINATED_BLOCK:
                     msg.Put("Block not properly terminated");
                     break;
                default:
                     assert(false);
            }

...
Grammar::Grammar(Control *control_) : control(control_),
                                      option(control_ -> option),
                                      lex_stream(control_ -> lex_stream),
                                      action_blocks(&(control_ -> option -> ActionBlocks())),
                                      variable_table(control_ -> variable_table),
                                      macro_table(control_ -> macro_table),
                                      parser(control_, control_ -> lex_stream, control_ -> variable_table, control_ -> macro_table),

                                      symbol_index(4, control_ -> variable_table -> Size()),
                                      rules(2, 2048),
                                      rhs_sym(4, 8192),
                                      name(2, control_ -> variable_table -> Size())
{
    return_code = 0;
...
void Grammar::ProcessInput(Action *action)
{
    parser.Parse();
    if (parser.rules.Length() == 0)
    {
        TextBuffer msg;
        msg.Put(option -> grm_file);
        msg.Put(" is an empty grammar... Processing stopped");
...
// nonterminals.
//
void Grammar::OptimizeNames(Base *base, Produce *produce)
{
    //
    // The "name" and "prostheses" maps are only needed for error recovery.
    //
    if (! (option -> error_maps &&
           option -> table != Option::NONE &&
           option -> names == Option::OPTIMIZED))
        return;
/*
int total = 0;
...
for (int symbol = FirstNonTerminal(); symbol <= LastNonTerminal(); symbol++)
{
    if (produce -> IsUseful(symbol))
    {
        total += base -> string_generated[symbol].Length();
        int rule_no = base -> prosthesis_rule[symbol];
        cout << "    " << RetrieveString(symbol) << " (" << rule_no << ")" << " : ";
        if (base -> string_generated[symbol].Length() == 1 && base -> string_generated[symbol][0] == OMEGA)
            cout << "???";
        else
        {
            if (base -> string_generated[symbol].Length() > 1)
            {
                for (int k = FirstRhsIndex(rule_no); k < EndRhsIndex(rule_no); k++)
                {
                    int sym = rhs_sym[k];
                    if (IsTerminal(sym))
                         cout << RetrieveString(sym) << " ";
                    else if (! base -> IsNullable(sym))
                         cout << RetrieveString(sym) << " ";
                }
                cout << "  ---  ";
            }
            for (int i = 0; i < base -> string_generated[symbol].Length(); i++)
            {
                assert(base -> string_generated[symbol][i] != OMEGA);
                cout << RetrieveString(base -> string_generated[symbol][i]) << " ";
            }
        }
        cout << "\n";
    }
...
*/
    ...
    {
        if (! produce -> IsUseful(symbol))
            symbol_index[symbol].external_name_index = symbol_index[accept_image].external_name_index;
        else if (symbol_index[symbol].external_name_index < 0)
        {
            if (base -> IsNullable(symbol))
                 symbol_index[symbol].external_name_index = symbol_index[accept_image].external_name_index;
            else symbol_index[symbol].external_name_index = - symbol_index[symbol].external_name_index;
        }
    }
    ...
    //
    Array<bool> name_used(num_names + 1, false);
    {
        for (int symbol = 1; symbol <= num_symbols; symbol++)
            name_used[symbol_index[symbol].external_name_index] = true;
    }
    ...
    Array<int> names_map(num_names + 1);
    for (int i = 0; i <= num_names; i++)
    {
        if (name_used[i])
        {
            names_map[i] = new_names.Length();
            new_names.Next() = name[i];
        }
    }
    ...
    {
        for (int symbol = 1; symbol <= num_symbols; symbol++)
            symbol_index[symbol].external_name_index = names_map[symbol_index[symbol].external_name_index];
    }
/*
{
for (int symbol = 1; symbol <= num_symbols; symbol++)
{
    if (symbol_index[symbol].external_name_index != symbol_index[accept_image].external_name_index)
    {
        cout << "Candidate " << RetrieveString(symbol) << "\n";
        cout.flush();
    }
}
}
...
//
int Grammar::GetProsthesisNameIndex(int lhs_index, int lhs_image, int classname_index)
{
    //
    // if classname_index is already a valid name, return it.
    //
    if (classname_index == 0) // || lex_stream -> NameStringLength(classname_index) == 1)
    {
        const char *name = RetrieveString(lhs_image),
                   *suffix = "Prosthesis";
        int length = strlen(name) + strlen(suffix) + 1;
        char *macro_name = new char[length + 1];
            macro_name[0] = option -> escape;
            macro_name[1] = '\0';
            strcat(macro_name, name);
            strcat(macro_name, suffix);
            ...
            else
            {
                ...
                msg.Put("\"");
                msg.Put(name);
                msg.Put("\" was previously used as a classname for a rule");
                option -> EmitWarning(lhs_index, msg);
            }
        delete [] macro_name;

        Token *loc_token = lex_stream -> GetTokenReference(classname_index > 0 ? classname_index : lhs_index);

        classname_index = lex_stream -> GetNextToken(loc_token -> FileSymbol(), loc_token -> StartLocation());

        Token *classname_token = lex_stream -> GetTokenReference(classname_index);
        classname_token -> SetEndLocation(loc_token -> EndLocation());
        classname_token -> SetKind(TK_MACRO_NAME);
        classname_token -> SetSymbol(macro_symbol);
    }
...
//
//
int Grammar::GetClassnameIndex(int rule_index, int lhs_index, int classname_index, int array_element_type_index)
{
    int lhs_image = rules[rule_index].lhs;

    Symbol *symbol;
    {
        const char *name = (classname_index > 0
                               ? lex_stream -> NameString(classname_index)
                               : RetrieveString(lhs_image));
        ...
        if (! Code::IsValidVariableName(name))
        {
            ...
            msg.Put("\"");
            msg.Put(name);
            msg.Put("\" is an illegal variable name");
            option -> EmitError((classname_index > 0
                                                 ? classname_index
                                                 : lhs_index), msg);
            return_code = 12;
        }
        ...
        else
        {
            ...
            {
                symbol = classname_set.FindName(name + 1, length - 1);
                if (symbol == NULL)
                    symbol = classname_set.FindOrInsertName(name, length);
                else
                {
                    TextBuffer msg;
                    msg.Put("The class name \"");
                    msg.Put(symbol -> Name());
                    msg.Put("\" was implicitly used previously as a class name");
                    option -> EmitWarning(classname_index, msg);
                }
            }
            else
            {
                char *new_name = new char[length + 2];
                {
                    new_name[0] = option -> escape;
                    strcpy(new_name + 1, name);
                    symbol = classname_set.FindName(new_name, length + 1);
                    if (symbol == NULL)
                        symbol = classname_set.FindOrInsertName(name, length);
                    else
                    {
                        ...
                        msg.Put("This name \"");
                        msg.Put(name);
                        msg.Put("\" was explicitly used previously as a class name");
                        option -> EmitWarning(lhs_index, msg);
                    }
                }
                delete [] new_name;
...
//
//
void Grammar::ProcessActions(Action *action, Base *base, Produce *produce)
{
    ...
    //
    BoundedArray<int>list_nonterminals(FirstNonTerminal(), LastNonTerminal(), 0);
    if (option -> automatic_ast != Option::NONE)
    {
        ...
        for (int rule_index = start_symbol.Length(); rule_index <= num_rules; rule_index++)
        {
            int k = rules[rule_index].source_index,
                ...
                index = GetClassnameIndex(rule_index,
                                          parser.rules[k].lhs_index,
                                          parser.rules[k].classname_index,
                                          parser.rules[k].array_element_type_index);

            if (classname[index].array_element_type_symbol != NULL &&
                (list_nonterminals[lhs_image] == 0 || list_nonterminals[lhs_image] == index))
                 list_nonterminals[lhs_image] = index;
            else list_nonterminals[lhs_image] = -1;
    ...
    //
    int save_parser_rules_length = parser.rules.Length(),
        save_rules_length = rules.Length(),
        save_rhs_sym_length = rhs_sym.Length();
    if (option -> error_maps)
    {
        char tok[Control::SYMBOL_SIZE + 1];
        rules.Reset(save_rules_length - 1); // remove the previous gate

        //
        //
        //
        prosthesis_rule_index.Resize(num_symbols + 1);
        prosthesis_rule_index.Initialize(0);
        {
            //
            // Assign a new rule number to each nonterminal for which a prosthesis is explicitly requested.
            //
            for (int k = 0; k < parser.prostheses.Length(); k++)
            {
                Tuple<ProcessedRuleElement> &processed_rule_elements = processed_rule_map.Next();
                VariableSymbol *lhs_symbol = lex_stream -> GetVariableSymbol(parser.prostheses[k].lhs_index);
                int lhs_image = lhs_symbol -> SymbolIndex();
                if (lhs_image == 0)
                {
                    TextBuffer msg;
                    msg.Put("Prosthesis requested for undeclared nonterminal: ");
                    msg.Put(lhs_symbol -> Name());
                    option -> EmitError(parser.prostheses[k].lhs_index, msg);
                    return_code = 12;
                    continue;
                }
                ...
                {
                    RestoreSymbol(tok, RetrieveString(lhs_image));
                    TextBuffer msg;
                    msg.Put("Terminal symbol \"");
                    msg.Put(tok);
                    msg.Put("\" used the left-hand side of a rule");
                    option -> EmitError(parser.prostheses[k].lhs_index, msg);
                    return_code = 12;
                    continue;
                }

                if (prosthesis_rule_index[lhs_image] != 0)
                {
                    RestoreSymbol(tok, RetrieveString(lhs_image));
                    TextBuffer msg;
                    msg.Put("Respecification of a prosthesis for nonterminal \"");
                    msg.Put(tok);
                    msg.Put("\" ignored");
                    option -> EmitWarning(parser.prostheses[k].lhs_index, msg);
                    continue;
                }

                if (symbol_index[lhs_image].external_name_index == symbol_index[accept_image].external_name_index)
                {
                    RestoreSymbol(tok, RetrieveString(lhs_image));
                    TextBuffer msg;
                    msg.Put("It is not necessary to specify a prosthesis for nonterminal \"");
                    msg.Put(tok);
                    msg.Put("\"");
                    option -> EmitWarning(parser.prostheses[k].lhs_index, msg);
                }
                ...
                //
                //
                parser.prostheses[k].classname_index = GetProsthesisNameIndex(parser.prostheses[k].lhs_index, lhs_image, parser.prostheses[k].classname_index);
                parser.rules.Next() = parser.prostheses[k];

                prosthesis_rule_index[lhs_image] = rules.NextIndex();
            }
            ...
            //
            if (option -> prostheses != Option::MINIMUM)
            {
                for (int lhs_image = FirstNonTerminal(); lhs_image <= LastNonTerminal(); lhs_image++)
                {
                    //
                    ...
                    //
                    if (prosthesis_rule_index[lhs_image] == 0 && produce -> IsUseful(lhs_image) && 
                        (option -> prostheses == Option::MAXIMUM || (! base -> IsNullable(lhs_image))))
                    {
                        int rule_no = RulesOfLhs(lhs_image)[0], // the first rule specified for lhs_image
                            source_index = rules[rule_no].source_index;
                        if (option -> automatic_ast != Option::NONE) // automatically create prosthesis.
                        {
                            jikespg_act::RuleDefinition &prosthesis = parser.rules.Next();
                            prosthesis.lhs_index = parser.rules[source_index].lhs_index;
                            if (list_nonterminals[lhs_image] > 0) // nonterminal associated with a list?
                            {
                                prosthesis.classname_index = parser.rules[source_index].classname_index;
                                prosthesis.array_element_type_index = parser.rules[source_index].array_element_type_index;
                            }
                            else
                            {
                                prosthesis.classname_index = GetProsthesisNameIndex(prosthesis.lhs_index, lhs_image, 0);
                                prosthesis.array_element_type_index = 0;
                            }

                            prosthesis.separator_index = parser.rules[source_index].separator_index;
                            prosthesis.end_rhs_index = lex_stream -> Next(prosthesis.separator_index);
                        ...
                        else
                        {
                            RestoreSymbol(tok, RetrieveString(lhs_image));
                            TextBuffer msg;
                            msg.Put("No prosthesis specified for \"");
                            msg.Put(tok); 
                            msg.Put("\"");
                            option -> EmitWarning(parser.rules[source_index].lhs_index, msg);
                        }
                    }
                }
            }
        ...
        //
        for (int k = save_parser_rules_length; k < parser.rules.Length(); k++)
        {
            VariableSymbol *lhs_symbol = lex_stream -> GetVariableSymbol(parser.rules[k].lhs_index);
            int lhs_image = lhs_symbol -> SymbolIndex(),
                rule_index = prosthesis_rule_index[lhs_image];
            ...
            //
            int last_symbol_index = lex_stream -> Previous(parser.rules[k].end_rhs_index);
            if (lex_stream -> Kind(last_symbol_index) == TK_BLOCK)
                last_symbol_index = lex_stream -> Previous(last_symbol_index);
            ...
            rules[rule_index].separator_token_kind = 0;
            rules[rule_index].rhs_index = rhs_sym.Length();
            rules[rule_index].source_index = k;
            rules[rule_index].produces_token_kind = 0;

            int i = lex_stream -> Previous(parser.rules[k].end_rhs_index);
            ...
            else if (option -> automatic_ast == Option::NONE) // not automatic AST? must supply code to build prosthesis
            {
                RestoreSymbol(tok, RetrieveString(lhs_image));
                TextBuffer msg;
                msg.Put("No action specified to create prosthesis for \"");
                msg.Put(tok); 
                msg.Put("\"");
                option -> EmitError(parser.rules[k].lhs_index, msg);
                return_code = 12;
            }
        }
        ...
        if (option -> automatic_ast != Option::NONE)
        {
            ...
            //
            for (int rule_index = num_rules + 1; rule_index < rules.Length() - 1; rule_index++)
            {
                int k = rules[rule_index].source_index,
                    index = GetClassnameIndex(rule_index,
                                              parser.rules[k].lhs_index,
    ...
    else if (parser.types.Length() > 0)
    {
        ...
        if (option -> warnings && untyped.Length() > 0)
        {
            ...
            for (int i = 0; i < untyped.Length(); i++)
            {
                ...
                TextBuffer msg;
                msg.Put("No type specified for nonterminal \"");
                msg.Put(tok);
                msg.Put("\". ");
                msg.Put(nonterminal_default_type);
                msg.Put(" assumed.");
    ...
    //
    notice_actions.Resize(0);
    header_actions.Resize(0);
    initial_actions.Resize(0);
    code_actions.Resize(0);
    ast_actions.Resize(0);
    trailer_actions.Resize(0);
    attribute_actions.Resize(0);
    classname_set.Reset();
    classname.Resize(0);
    special_nonterminal_array.Resize(0);
    processed_rule_map.Resize(0);
    ...
    //
    rules.Reset(save_rules_length - 1);           // restore the original rules
    rules.Next().rhs_index = rhs_sym.Length();    // add a gate
    rhs_sym.Reset(save_rhs_sym_length);
    parser.rules.Reset(save_parser_rules_length);
...
void Grammar::ProcessExportedTerminals(void)
{
    ...
    for (int i = 0; i < imports.Length(); i++)
    {
        ...
        else
        {
            ...
            msg.Put("Symbol \"");
            msg.Put(symbol -> Name());
            msg.Put("\" was specified more than once in an Export section. Previous specification was located at ");
            msg.Put(lex_stream -> FileName(decl));
            msg.Put(":");
            msg.Put(lex_stream -> Line(decl));
            msg.Put(":");
            msg.Put(lex_stream -> Column(decl));
            msg.Put(":");
            msg.Put(lex_stream -> EndLine(decl));
            msg.Put(":");
            msg.Put(lex_stream -> EndColumn(decl));
            msg.Put(":");
            msg.Put(lex_stream -> StartLocation(decl));
            msg.Put(":");
            msg.Put(lex_stream -> EndLocation(decl));
...
void Grammar::ProcessTerminals(Tuple<int> &declared_terminals)
{
    ...
    for (int i = 1; i < parser.start_indexes.Length(); i++)     // Create the terminal markers for the extra entry points, if any.
    {
        ...
        char *marker_name = new char[length];
        {
            ...
            {
                TextBuffer msg;
                msg.Put("Unable to declare a marker for symbol \"");
                msg.Put(entry_name);
                msg.Put("\". After trying the suffixes \"Marker\" and \"Marker\" + [\"00\"..\"99\"]");
        ...
        if (num_undeclared_terminals > 0) // Some terminals were not declared?
        {
            ...
            for (int symbol = 1; symbol <= num_terminals; symbol++)
            {
                if (! declared_set[symbol])
                {
                    ...
                    TextBuffer msg;
                    msg.Put("The undeclared symbol \"");
                    msg.Put(tok);
                    msg.Put("\" is assumed to be a terminal");
    ...
    if (option -> warnings && option -> import_file.Length() > 0)
    {
        ...
        for (int o = 0; o < variable_table -> Size(); o++)
        {
            ...
            if (term_set[symbol -> Index()] && (! imported_term_set[symbol -> Index()]))
            {
                ...
                TextBuffer msg;
                msg.Put("The terminal symbol \"");
                msg.Put(tok);
                msg.Put("\" was not imported from");
                if (option -> import_file.Length() == 1)
                {
                    msg.Put(" ");
                    msg.Put(option -> import_file[0]);
                }
                else
                {
                    msg.Put(": ");
...
void Grammar::ProcessRemainingAliases(Tuple<int> &remaining_aliases)
{
    for (int k = 0; k < remaining_aliases.Length(); k++)
    {
        ...
        {
            TextBuffer msg;
            msg.Put(lex_stream -> NameString(parser.aliases[i].rhs_index));
            msg.Put(" can\'t be used as an alias for \"");
            msg.Put(lex_stream -> NameString(parser.aliases[i].lhs_index));
            msg.Put("\" because it was not defined.\n");
...
char *Grammar::InsertInterface(SymbolLookupTable &symbol_set, const char *name)
{
...
void Grammar::ProcessRules(Tuple<int> &declared_terminals)
{
    ...
    {
        TextBuffer msg;
        msg.Put("The start symbol, \"");
        msg.Put(start -> Name());
        msg.Put("\", is aliased to the Empty symbol");
    ...
    //
    special_nonterminal_array.Resize(variable_table -> Size());
    {
        //
    ...
    //
    {
        for (int k = 0; k < parser.prostheses.Length(); k++)
        {
            ...
            VariableSymbol *lhs_symbol = lex_stream -> GetVariableSymbol(parser.prostheses[k].lhs_index);
            int lhs_image = lhs_symbol -> SymbolIndex();
            if (IsTerminal(lhs_image)) // error that will be processed later.
                continue;

            int i = lex_stream -> Previous(parser.prostheses[k].end_rhs_index);
            if (lex_stream -> Kind(i) == TK_BLOCK)
            {
                //
                // Keep track of all pairs (nonterminal X array_type) that are associated with
                // a rule that contain actions.
                //
                int array_element_type_index = parser.prostheses[k].array_element_type_index;
    ...
    //
    lhs_rule.Resize(num_terminals + 1, num_symbols);
    for (int rule_no = FirstRule(); rule_no <= LastRule(); rule_no++)
    {
...
//
void Grammar::RestoreSymbol(char *out, const char *in)
{
    int len = strlen(in);
    if (len > 0)
    {
...
// NOTE that it is assumed that IN and OUT do not overlap each other.
//
void Grammar::RestoreSymbol(TextBuffer &buffer, char *in)
{
    int len = strlen(in);
    if (len > 0)
    {
...
//
void Grammar::DisplayNameMap(int symbol)
{
    ...
    PrintLargeToken(line, tok, "", len);
    strcat(line, " ::= ");
    RestoreSymbol(tok, RetrieveName(symbol));
...
//
void Grammar::ProcessGrammarOutput(Action *action)
{
    if (! option -> list)
        return;
    ...
    if (option -> error_maps)
    {
        ...
        {
            for (int symbol = 1; symbol <= num_symbols; symbol++)
                if (symbol != accept_image && symbol_index[symbol].external_name_index == symbol_index[accept_image].external_name_index)
                    DisplayNameMap(symbol);
        }
    }

...
/************************** Unneeded Test? ****************************
        if (count[grammar -> rules[i].lhs] == 99)
        {
            ...
            msg.Put("Nonterminal \"");
            msg.Put(name);
            msg.Put("\" produces more than 99 rules.");
            option -> EmitError(0, msg);
...
void PlxTable::print_symbols(void)
{
    ...
    for (symbol = grammar -> FirstTerminal(); symbol <= grammar -> LastTerminal(); symbol++)
    {
        ...
        if (tok[0] == '\n' || tok[0] == option -> escape)
        {
            ...
            msg.Put(tok);
            msg.Put(" may be an invalid PLX variable.");
        ...
        else if (strpbrk(tok, "!%^&*()-+={}[];:\"`~|\\,.<>/?\'") != NULL)
        {
            TextBuffer msg;
            msg.Put(tok);
            msg.Put(" is an invalid PLX variable name.");
...
void PlxTable::print_definitions(void)
{
    if (option -> error_maps)
    {
        fprintf(sysdef, " %%dcl ERROR_SYMBOL      FIXED; %%ERROR_SYMBOL = %d;\n"
                        ...
                        grammar -> error_image,
                        produce -> scope_prefix.Size() - 1,
                        produce -> scope_prefix.Size(),
                        max_name_length,
                        max_name_length,
                        dfa -> num_states);
    }
...
//
//
void PlxTable::OutputTables(void)
{
    init_parser_files();

...
void Option::EmitError(int index, TextBuffer &msg)       { Emit(lex_stream -> GetTokenReference(index), "Error: ", msg); }
void Option::EmitWarning(int index, const char *msg)     { Emit(lex_stream -> GetTokenReference(index), "Warning: ", msg); }
void Option::EmitWarning(int index, TextBuffer &msg)     { Emit(lex_stream -> GetTokenReference(index), "Warning: ", msg); }
void Option::EmitInformative(int index, const char *msg) { Emit(lex_stream -> GetTokenReference(index), "Informative: ", msg); }
void Option::EmitInformative(int index, TextBuffer &msg) { Emit(lex_stream -> GetTokenReference(index), "Informative: ", msg); }
...
void Option::Emit(Token *token, const char *header, TextBuffer &msg)
{
    Emit(token, token, header, msg);
...
void Option::Emit(Token *startToken, Token *endToken, const char *header, TextBuffer &msg)
{
    EmitHeader(startToken, endToken, header);
    report.Put(msg);
    report.PutChar('\n');
...
void Option::InvalidValueError(const char *start, const char *value, int length)
{
    ...
    msg.Put("\"");
    msg.Put(value);
    msg.Put("\" is an invalid value for option \"");
    msg.Put(str);
    msg.Put("\"");
...
void Option::InvalidTripletValueError(const char *start, int length, const char *type, const char *format)
{
    ...
    msg.Put("Illegal ");
    msg.Put(type);
    msg.Put(" option specified: ");
    msg.Put(str);
    msg.Put(". A value of the form \"");
    msg.Put(format);
    msg.Put("\" was expected.");
    EmitError(GetTokenLocation(start, length), msg);
}
    ...
    if (flag)
    {
        ...
        if (start[i + 1] == '=' || IsDelimiter(start[i + 1]))
        {
            ...
            else
            {
                ...
                {
                    if (strxsub(value, "maximum") == length)
                        names = MAXIMUM;
                    else
                    {
                        i = strxsub(value, "minimum");
                        if (i == 1)
...
const char *Option::ClassifyP(const char *start, bool flag)
{
    ...
    //                *q = ValuedOption(p);
    //     if (i < 1)
    //          return ReportAmbiguousOption(start, "PACKAGE, PARSETABLE-INTERFACES, PREFIX, PRIORITY, PROGRAMMING_LANGUAGE, PROSTHESES, PRS_FILE");
    //     else if (i < 2)
    ...
    if (flag)
    {
        ...
        if (start[i + 1] == '=' || IsDelimiter(start[i + 1]))
        {
            const char *p = start + i + 1,
                       *q = ValuedOption(p);
            if (i < 1)
                 return ReportAmbiguousOption(start, "PACKAGE, PARENT_SAVED, PARSETABLE-INTERFACES, PREFIX, PRIORITY, PROGRAMMING_LANGUAGE, PROSTHESES, PRS_FILE");
            else if (i < 2)
        ...
        if (start[i + 1] == '=' || IsDelimiter(start[i + 1]))
        {
            const char *p = start + i + 1,
                       *q = ValuedOption(p);
            if (i < 2)
                 return ReportAmbiguousOption(start, "PREFIX, PRIORITY, PROGRAMMING_LANGUAGE, PROSTHESES, PRS_FILE");
            else if (q == NULL) // Must have a value
        ...
        if (start[i + 1] == '=' || IsDelimiter(start[i + 1]))
        {
            const char *p = start + i + 1,
                       ...
                       *value;
            if (i < 3)
                 return ReportAmbiguousOption(start, "PROGRAMMING_LANGUAGE, PROSTHESES");
            else if (q == NULL) // Must have a value
        ...
        i = Max(strxsub(start + 1, "rostheses"), strxsub(start + 1, "rosthesis"));
        if (start[i + 1] == '=' || IsDelimiter(start[i + 1]))
        {
            const char *p = start + i + 1,
                       *q = ValuedOption(p);
            if (q == NULL) // Must have a value
                return ReportMissingValue(start, "PROSTHESES");
            else
            {
                const char *value;
                p = GetStringValue(q, value);
...
const char *Option::ReportMissingValue(const char *start, const char *option)
{
    ...
    TextBuffer msg;
    msg.Put("A value is required for this option: \"");
    msg.Put(str);
    msg.Put("\"");
    EmitError(GetTokenLocation(start, length), msg);
...
const char *Option::ReportValueNotRequired(const char *start, const char *option)
{
    ...
    TextBuffer msg;
    msg.Put("An illegal value was specified for this option: \"");
    msg.Put(str);
    msg.Put("\"");
    EmitError(GetTokenLocation(start, length), msg);
...
const char *Option::GetStringValue(const char *start, const char *&value)
{
    ...
    if (quote != NULL_CHAR)
    {
        ...
        else // an unterminated string
        {
            ...
            msg.Put("The string ");
            msg.Put(str);
            msg.Put("\" was not properly terminated");
            EmitError(GetTokenLocation(start, length), msg);
        }
    }
    else
...
void Option::ProcessBlock(BlockInfo &block_info)
{
    ...
    {
        msg.Put("The action filename \"");
        msg.Put(filename);
        msg.Put("\" was previously associated with the begin block marker \"");
        msg.Put(filename_symbol -> Block() -> Name());
        msg.Put("\" and cannot be reassociated with the block marker \"");
        msg.Put(block_info.block_begin);
        msg.Put("\"");
        EmitError(block_info.location, msg);
    }
    else if (action_blocks.FindBlockname(block_info.block_begin, block_begin_length) != NULL)
    {
        msg.Put("The action block begin string \"");
        msg.Put(block_info.block_begin);
        msg.Put("\" was used in a previous block definition");
        EmitError(block_info.location, msg);
    }
    else
    {
...
void Option::ProcessHeader(BlockInfo &block_info)
{
    ...
    if (filename_symbol != NULL)
    {
        if (action_blocks.FindBlockname(block_info.block_begin, strlen(block_info.block_begin)) != NULL)
        {
            msg.Put("The action block begin string \"");
            msg.Put(block_info.block_begin);
            msg.Put("\" was used in a previous definition");
            EmitError(block_info.location, msg);
        }
        else
        {
    ...
    else
    {
        msg.Put("The action filename \"");
        msg.Put(filename);
        msg.Put("\" must be associated with an action block before being used here");
        EmitError(block_info.location, msg);
    }
...
void Option::CheckBlockMarker(Token *marker_location, const char *block_marker)
{
    ...
    if (block_marker_length == 1)
    {
        ...
        {
            msg.Put("The escape symbol, \"");
            msg.Put(block_marker);
            msg.Put("\", cannot be used as a block marker.");
            EmitError(marker_location, msg);
        }
        else if (*block_marker == or_marker)
        {
            msg.Put("The OR_MARKER symbol, \"");
            msg.Put(block_marker);
            msg.Put("\", cannot be used as a block marker.");
            EmitError(marker_location, msg);
        }
...
void Option::CheckGlobalOptionsConsistency()
{
    ...
    if (or_marker == escape)
    {
         ...
         msg.Put("The escape symbol, \"");
         msg.Put(str);
         msg.Put("\", cannot be used as the OR_MARKER");
        ...
        for (int i = 0; i < blocks.Length(); i++)
        {
            ...
            for (int k = i + 1; k < blocks.Length(); k++)
            {
                ...
                if (block1 -> BlockBeginLength() < block2 -> BlockBeginLength())
                {
                    ...
                    {
                        msg.Reset();
                        msg.Put("The block marker \"");
                        msg.Put(block1 -> BlockBegin());
                        msg.Put("\" is a substring of the block marker \"");
                        msg.Put(block2 -> BlockBegin());
                        msg.Put("\"");
...
void Option::CheckAutomaticAst()
{
    ...
    else
    {
        ...
        while (i < directory_length)
        {
            ...
            {
                msg.Reset();
                msg.Put("The subpackage \"");
                msg.Put(name);
                msg.Put("\" specified in the ast_directory option is not a valid Java variable name");
            ...
            {
                msg.Reset();
                msg.Put("Unable to create ast directory: \"");
                msg.Put(temp);
                msg.Put("\"");
                EmitError(ast_directory_location, msg);
...
void Option::CheckDirectory(Token *directory_location, const char *directory)
{
    ...
    char *temp = new char[directory_length + 2];
    {
        ...
        while (i < directory_length)
        {
            ...
            {
                msg.Reset();
                msg.Put("Unable to create directory: \"");
                msg.Put(temp);
                msg.Put("\"");
...
void Option::CompleteOptionProcessing()
{
    ...
    if (dcl_file == NULL)
    {
        dcl_file = GetFile(out_directory,
                           (programming_language == C || programming_language == CPP) ? "prs." : "dcl.",
                           (programming_language == JAVA
                                ? "java"
                                : (programming_language == ML
                                       ? "ml"
                                                         ...
                                                         : (programming_language == PLX
                                                                ...
                                                                : (programming_language == PLXASM
                                                                        ...
                                                                        : (programming_language == ML
                                                                               ? "PROGRAMMING_LANGUAGE=ML"
                                                                               : "PROGRAMMING_LANGUAGE=XML"))))));
    opt_string.Next() = AllocateString(prostheses == MAXIMUM
                                                   ? "PROSTHESES=MAXIMUM"
                                                   : (prostheses == MINIMUM
                                                                  ? "PROSTHESES=MINIMUM"
                                                                  : "PROSTHESES=OPTIMIZED"));
...
void Option::PrintOptionsList(void)
{
    cout << "\n"
         ...
         <<    "\n(C) Copyright IBM Corp. 1984, 2006.\n"
               ...
               "-prostheses=<maximum|minimum|optimized>               " "\n"
               "-prs-file=string                                      " "\n"
               "-quiet                                                " "\n"
               "-read-reduce                                          " "\n"
               "-remap-terminals                                      " "\n"

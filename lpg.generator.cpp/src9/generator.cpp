...
{
    Dfa::ShiftHeader &sh = dfa -> shift[shift_no];
...
void Generator::compute_default_shift(void)
{
    ...
    for (int state_no = 1; state_no <= pda -> max_la_state; state_no++)
        process_shift_actions(action_count, dfa -> statset[state_no].shift_number);
    ...
    {
        report.PutChar('\n');
        report.Put("Number of Shift entries saved by default: ");
        report.Put(shift_count);
        report.PutChar('\n');
        report.Put("Number of Shift/Reduce entries saved by default: ");
        report.Put(shift_reduce_count);
        report.PutChar('\n');
        if (option -> backtrack)
        {
            report.Put("Number of conflict entries saved by default: ");
            report.Put(conflict_count);
            report.PutChar('\n');
        }
    }

    dfa -> num_shifts -= shift_count;
    dfa -> num_shift_reduces -= shift_reduce_count;
...
void Generator::compute_default_goto(void)
{
    ...
    for (int state_no = 1; state_no <= dfa -> num_states; state_no++)
    {
        Dfa::GotoHeader &go_to = dfa -> statset[state_no].go_to;
...
assert(default_action != 0);
    ...
    {
        for (int state_no = 1; state_no <= dfa -> num_states; state_no++)
            dfa -> statset[state_no].go_to.RemoveDefaults(default_goto);
    }

    if (! option -> quiet)
    {
        report.PutChar('\n');
        report.Put("Number of Goto entries saved by default: ");
        report.Put(goto_count);
        report.PutChar('\n');
        report.Put("Number of Goto/Reduce entries saved by default: ");
        report.Put(goto_reduce_count);
        report.PutChar('\n');
    }

    dfa -> num_gotos -= goto_count;
    dfa -> num_goto_reduces -= goto_reduce_count;
...
//
void Generator::ProcessCompressionAndGeneration(void)
{
    num_entries = pda -> max_la_state + dfa -> num_shifts + dfa -> num_shift_reduces
                                      + dfa -> num_gotos  + dfa -> num_goto_reduces
                                      + pda -> num_reductions;
    ...
    for (int state_no = 1; state_no <= pda -> max_la_state; state_no++)
    {
        ...
        Dfa::ReduceHeader &red = dfa -> statset[state_no].reduce;
        {
            for (int i = 1; i < red.Length(); i++)
                red[i].SetSymbol(red[i].Symbol() - 1);
        }

        Dfa::ConflictHeader &conflict = dfa -> statset[state_no].conflict;
        {
    ...
    {
        for (int i = 0; i < produce -> gd_range.Length(); i++)
            produce -> gd_range[i]--;
    }

    {
        for (int i = 0; i < produce -> scope_lhs_symbol.Size(); i++)
        {
            produce -> scope_lhs_symbol[i]--;
            produce -> scope_look_ahead[i]--;
        }
    }

    {
        for (int i = 1; i < produce -> scope_right_side.Size(); i++)
            if (produce -> scope_right_side[i] != 0)
                produce -> scope_right_side[i]--;
    }
    ...
    //
    {
        for (int i = 1; i <= dfa -> num_shift_maps; i++)
        {
            Dfa::ShiftHeader &sh = dfa -> shift[i];
    ...
    //
    // We update the prostheses_rule_indexes map.
    //
    if (option -> error_maps)
    {
        for (int symbol = grammar -> FirstNonTerminal(); symbol <= grammar -> LastNonTerminal(); symbol++)
            grammar -> prosthesis_rule_index[symbol] = grammar -> prosthesis_rule_index[symbol + 1];
    }
/*
// TODO: Remove this ... Debug info for prostheses
if (grammar -> prosthesis_rule_index.Length() > 0)
{
cout << "\n\nProsthesis info:\n";
for (int i = grammar -> FirstNonTerminal(); i <= grammar -> LastNonTerminal(); i++)
{
cout << i << " " << grammar -> RetrieveString(i) << " " << grammar -> RetrieveName(i) << " " << grammar -> prosthesis_rule_index[i] << "\n";
cout.flush();
}
}
*/
    ...
    if (option -> error_maps)
    {
        naction_symbols.Resize(dfa -> num_states + 1);
        for (int i = 1; i <= dfa -> num_states; i++)
            naction_symbols[i].Initialize(grammar -> num_nonterminals + 1, grammar -> num_terminals);

        action_symbols.Resize(dfa -> num_states + 1);
        {
            for (int i = 1; i <= dfa -> num_states; i++)
    ...
    for (int i = 1; i <= dfa -> num_states; i++)
    {
        ...
        if (state_start[state_no] > 0)
        {
                 ...
                 ! end_node; end_node = (state == state_no))
            {
                ...
                Dfa::ShiftHeader &sh = dfa -> Shift(state);
                for (int j = 0; j < sh.Length(); j++)
                {
                ...
                Dfa::ReduceHeader &red = dfa -> statset[state].reduce;
                {
                                              ...
                                              Array<int> &naction_symbols_range)
{
    ...
    for (int i = 1; i <= dfa -> num_states; i++)
    {
        ...
        if (state_start[state_no] > 0)
        {
            ...
            {
                state = state_stack[state];
                for (int j = produce -> gd_index[state]; j <= produce -> gd_index[state + 1] - 1; j++)
                {
                    int symbol = produce -> gd_range[j];
                    if (symbol_list[symbol] == Util::OMEGA)
                    {
                        symbol_list[symbol] = symbol_root;
                        symbol_root = symbol;
...
void Generator::remap_non_terminals(void)
{
    ...
    Array<int> row_size(dfa -> num_states + 1);
    ...
    //
    Frequency::sort(frequency_symbol, frequency_count, dfa -> num_states);
    ...
    if (option -> goto_default)
    {
        ...
        if (! option -> quiet)
        {
            report.Put("Number of non-terminals eliminated: ");
            report.Put(grammar -> num_nonterminals - last_symbol);
            report.PutChar('\n');
        }
...
void Generator::InitializeTables(int num_table_entries, int minimum_increment)
{
    ...
    {
        TextBuffer msg;
        msg.Put("Table has exceeded maximum limit of ");
        msg.Put(MAX_TABLE_SIZE);
...
void Generator::Reallocate(int num_table_entries)
{
    ...
    if (option -> verbose)
    {
        report.Put("Reallocating storage for tables, adding ");
        report.Put(table_size - old_size);
        report.Put(" entries.\n");
    }
...
{
    int num_table_entries = dfa -> num_gotos + dfa -> num_goto_reduces + dfa -> num_states;
    ...
    for (int k = 1; k <= dfa -> num_states; k++)
    {
    ...
    error_act = accept_act + dfa -> conflicts.Length();
    ...
    if (! option -> quiet)
    {
        ...
        report.PutChar('\n');
        report.Put("Number of entries in base Action Table: ");
        report.Put(num_table_entries);
        report.PutChar('\n');
        report.Put("Additional space required for compaction of Action Table: ");
        report.Put(percentage / 10);
        report.PutChar('.');
        report.Put(percentage % 10);
        report.Put("%\n");
    }
...
void Generator::merge_similar_t_rows(void)
{
    ...
    Array<int> table(dfa -> num_shift_maps + 1, Util::NIL);
    ...
    for (int state_no = 1; state_no <= pda -> max_la_state; state_no++)
    {
        ...
        //
        if (state_no > dfa -> num_states && dfa -> statset[state_no].predecessors.Length() == 0)
            continue;
        ...
        //
        Dfa::ShiftHeader &sh = dfa -> Shift(state_no);
        ...
        if (index != Util::OMEGA)
        {
            ...
            {
                int k;
                for (k = act - pda -> max_la_state; dfa -> conflicts[k] != 0; k++)
                {
                    if (dfa -> conflicts[k] < 0 ||
                        dfa -> conflicts[k] > grammar -> num_rules)
                        break;
                }
                shift_on_error_symbol[state_no] = (dfa -> conflicts[k] != 0);
            }
        }

        int hash_address = dfa -> statset[state_no].shift_number,
            j;
...
void Generator::merge_shift_domains(void)
{
    ...
    Array<int> shift_domain_table(SHIFT_TABLE_SIZE, Util::NIL),
               ...
               ordered_shift(dfa -> num_shift_maps + 1),
    ...
    real_shift_number.Resize(dfa -> num_shift_maps + 1);
    ...
    int num_table_entries = 0;
    {
        for (int state_no = 1; state_no <= num_terminal_states; state_no++)
        {
            ...
            else
            {
                ...
                {
                    Dfa::ShiftHeader &sh = dfa -> shift[new_state_element[i].shift_number];
    ...
    //
    shift_check_index.Resize(dfa -> num_shift_maps + 1);
    for (int k = 0; k <= shift_domain_count; k++)
    {
        int shift_no = ordered_shift[k];
        Dfa::ShiftHeader &sh = dfa -> shift[real_shift_number[shift_no]];
    ...
    if (! option -> quiet)
    {
        ...
        report.PutChar('\n');
        report.Put("Number of entries in Shift Check Table: ");
        report.Put(num_table_entries);
        report.PutChar('\n');
        report.Put("Additional space required for compaction of Shift Check Table: ");
        report.Put(percentage /10);
        report.PutChar('.');
        report.Put(percentage % 10);
        report.Put("%\n");
...
int Generator::overlay_sim_t_rows(void)
{
    ...
    for (int i = multi_root; i != Util::NIL; i = new_state_element[i].thread)
    {
        ...
        if (state_root != Util::NIL)
        {
            ...
            new_state_element[index].shift_number = dfa -> statset[state_root].shift_number;
    ...
    {
        report.PutChar('\n');
        report.Put("Number of unique terminal states: ");
        report.Put(num_terminal_states);
        report.PutChar('\n');
        report.Put("Number of Shift actions saved by merging: ");
        report.Put(num_shifts_saved);
        report.PutChar('\n');
        report.Put("Number of Conflict points saved by merging: ");
        report.Put(num_conflicts_saved);
        report.PutChar('\n');
        report.Put("Number of Reduce actions saved by merging: ");
        report.Put(num_reductions_saved);
        report.PutChar('\n');
        report.Put("Number of Reduce saved by default: ");
        report.Put(default_saves);
        report.PutChar('\n');
    }
...
void Generator::overlap_t_rows(int num_table_entries)
{
    ...
    for (int k = 1; k <= num_terminal_states; k++)
    {
        ...
        //
        terminal_list.Reset();
        Dfa::ShiftHeader &sh = dfa -> shift[new_state_element[state_no].shift_number];
        for (int i = 0; i < sh.Length(); i++)
        {
            int symbol = sh[i].Symbol();
    ...
    if (! option -> quiet)
    {
        ...
        report.PutChar('\n');
        report.Put("Number of entries in Terminal Action Table: ");
        report.Put(num_table_entries);
        report.PutChar('\n');
        report.Put("Additional space required for compaction of Terminal Table: ");
        report.Put(percentage / 10);
        report.PutChar('.');
        report.Put(percentage % 10);
        report.Put("%\n");
...
void Generator::Generate(Table *parse_table)
{
    ...
    {
        TextBuffer msg;
        msg.Put("Table contains entries that are > ");
        msg.Put(MAX_TABLE_SIZE + 1);
        msg.Put("; Processing stopped.");
    ...
    lhs_and_baseAction.array.Resize(grammar -> num_rules + action_size + dfa -> conflicts.Length() + 1);
    ...
    if (option -> backtrack)
    {
        for (int i = 0; i < dfa -> conflicts.Length(); i++)
        {
            int act = dfa -> conflicts[i];
    ...
    for (int state_no = 1; state_no <= num_terminal_states; state_no++)
    {
        ...
        for (int j = 0; j < sh.Length(); j++)
        {
            ...
            if ((! option -> shift_default) || (act != default_shift[symbol]))
            {
                ...
                {
                    TextBuffer msg;
                    msg.Put("Table contains look-ahead shift entry that is > ");
                    msg.Put(MAX_TABLE_SIZE + 1);
                    msg.Put("; Processing stopped.");
    ...
    if (option -> shift_default)
    {
        ...
        shiftCheck.array.Initialize(Grammar::DEFAULT_SYMBOL);
        {
            for (int i = 1; i <= shift_domain_count; i++)
            {
                ...
                Dfa::ShiftHeader &sh = dfa -> shift[real_shift_number[i]];
                for (int j = 0; j < sh.Length(); j++)
                {
                    int symbol = sh[j].Symbol();
    ...
    if (option -> error_maps || option -> states)
    {
        ...
        if (option -> states)
        {
            ...
            //
            //
            parse_table -> original_state.Resize(dfa -> num_states + 1); // print only the regular (not lookahead) states
            {
            ...
            //
            //
            parse_table -> sorted_state.Resize(dfa -> num_states + 1);  // print only the regular (not lookahead) states
            {
    ...
    if (option -> error_maps)
    {
        ...
        //
        Array<int> as_size(dfa -> num_states + 1);
        for (int state_no = 1; state_no <= dfa -> num_states; state_no++)
        {
            Dfa::ShiftHeader &sh = dfa -> Shift(state_no);
        ...
        Array<int> state_start(dfa -> num_states + 2),
                   state_stack(dfa -> num_states + 1);
        ...
        action_symbols_base.type_id = parse_table -> Type(0, state_start[dfa -> num_states + 1]); // see action_symbols_range
        action_symbols_base.array.Resize(dfa -> num_states + 1);
        ...
        action_symbols_range.array.Resize(state_start[dfa -> num_states + 1]);
        ...
        //
        {
            for (int state_no = 1; state_no <= dfa -> num_states; state_no++)
            {
                as_size[state_no] = produce -> gd_index[state_no + 1] - produce -> gd_index[state_no];

                for (int i = produce -> gd_index[state_no]; i <= produce -> gd_index[state_no + 1] - 1; i++)
                     naction_symbols[state_no].AddElement(produce -> gd_range[i]);
            }
        }
        ...
        {
            for (int i = 0; i < produce -> gd_range.Length(); i++) // Remap non-terminals
                produce -> gd_range[i] = symbol_map[produce -> gd_range[i]] - grammar -> num_terminals;
        }
        ...
        naction_symbols_base.type_id = parse_table -> Type(0, state_start[dfa -> num_states + 1]); // see naction_symbols_range
        naction_symbols_base.array.Resize(dfa -> num_states + 1);
        ...
        naction_symbols_range.array.Resize(state_start[dfa -> num_states + 1]);
        ...
        //
        if (produce -> scope_prefix.Size() > 0)
        {
            Table::IntArrayInfo &scope_prefix = parse_table -> data.Next();
            scope_prefix.name_id = Table::SCOPE_PREFIX;
            scope_prefix.type_id = parse_table -> Type(0, produce -> scope_right_side.Size());
            scope_prefix.array.Resize(produce -> scope_prefix.Size());

            Table::IntArrayInfo &scope_suffix = parse_table -> data.Next();
            scope_suffix.name_id = Table::SCOPE_SUFFIX;
            scope_suffix.type_id = parse_table -> Type(0, produce -> scope_right_side.Size());
            scope_suffix.array.Resize(produce -> scope_suffix.Size());
            ...
            scope_lhs_symbol.array.Resize(produce -> scope_lhs_symbol.Size());
            ...
            scope_look_ahead.array.Resize(produce -> scope_look_ahead.Size());

            Table::IntArrayInfo &scope_state_set = parse_table -> data.Next();
            scope_state_set.name_id = Table::SCOPE_STATE_SET;
            scope_state_set.type_id = parse_table -> Type(0, produce -> scope_state.Size());
            scope_state_set.array.Resize(produce -> scope_state_set.Size());

            for (int j = 0; j < produce -> scope_look_ahead.Size(); j++)
            {
                scope_prefix.array[j]     = produce -> scope_prefix[j];
                scope_suffix.array[j]     = produce -> scope_suffix[j];
                scope_lhs_symbol.array[j] = symbol_map[produce -> scope_lhs_symbol[j]]
                                             - grammar -> num_terminals;
                scope_look_ahead.array[j] = symbol_map[produce -> scope_look_ahead[j]];
                scope_state_set.array[j]  = produce -> scope_state_set[j];
            }
            ...
            scope_right_side.array.Resize(produce -> scope_right_side.Size());
            for (int i = 0; i < scope_right_side.array.Size(); i++)
            {
                scope_right_side.array[i] = (produce -> scope_right_side[i] == 0
                                                  ? 0
                                                  : symbol_map[produce -> scope_right_side[i]]);
            }
            ...
            scope_state.array.Resize(produce -> scope_state.Size());
            for (int l = 0; l < produce -> scope_state.Size(); l++)
            {
                int state_no = produce -> scope_state[l];
            ...
            inSymb.array.Resize(dfa -> num_states + 1);
            inSymb.array[0] = 0;
            inSymb.array[1] = 0;
            for (int state_no = 2; state_no < inSymb.array.Size(); state_no++)
            {
                Node *q = dfa -> statset[state_no].kernel_items;
                int symbol = 0;
                if (q != NULL)
                {
        ...
        //
        // Build the prosthesis_rule_indexes map.
        //
        Table::IntArrayInfo &prosthesis_rule_index = parse_table -> data.Next();
        prosthesis_rule_index.name_id = Table::PROSTHESIS_RULE_INDEX;
        prosthesis_rule_index.type_id = parse_table -> Type(0, grammar -> num_rules + grammar -> num_nonterminals);
        prosthesis_rule_index.array.Resize(grammar -> num_nonterminals + 1);
        prosthesis_rule_index.array[0] = 0;
        for (int symbol = grammar -> FirstNonTerminal(); symbol <= grammar -> LastNonTerminal(); symbol++)
            prosthesis_rule_index.array[symbol_map[symbol] - grammar -> num_terminals] = grammar -> prosthesis_rule_index[symbol];

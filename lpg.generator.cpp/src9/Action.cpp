...
Action::~Action() { delete [] abstract_ast_list_classname; }
...
void Action::InsertExportMacros()
{
    ...
    for (int i = 0; i < grammar -> parser.exports.Length(); i++)
    {
        ...
        {
            TextBuffer msg;
            msg.Put("The name of the exported symbol ");
            msg.Put(macro -> Name());
            msg.Put(" conflicts with the predefined macro of the same name");
...
void Action::CheckExportMacros()
{
    for (int i = first_locally_exported_macro; i < locally_exported_macro_gate; i++)
    {
        ...
        {
            TextBuffer msg;
            msg.Put("The exported terminal symbol ");
            msg.Put(simple_macro -> Name() + 2);
            msg.Put(" was not generated by any rule in this grammar");
...
void Action::CheckMacrosForConsistency()
{
    for (int i = 0; i < macro_table -> Size(); i++)
    {
        ...
        {
            TextBuffer msg;
            msg.Put("The user-defined macro ");
            msg.Put(macro -> Name());
            msg.Put(" conflicts with the predefined macro of the same name");
        ...
        else if (FindExportMacro(macro -> Name(), macro -> NameLength()))
        {
            ...
            msg.Put("The user-defined macro ");
            msg.Put(macro -> Name());
            msg.Put(" conflicts with an exported symbol of the same name");
            option -> EmitError(macro -> Location(), msg);
                                   ...
                                   Array<RuleAllocationElement> &rule_allocation_map)
{
    ...
    for (int i = 0; i < classname.Length(); i++)
    {
        ...
        if (classname[i].array_element_type_symbol != NULL)
        {
            ...
            BoundedArray< Tuple<int> > nonterminal_map(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
            {
                for (int k = 0; k < rule.Length(); k++)
                {
                    int rule_no = rule[k],
            ...
            //
            BoundedArray<bool> is_list_nonterminal(grammar -> FirstNonTerminal(), grammar -> LastNonTerminal());
            is_list_nonterminal.MemReset();
            for (int k = 0; k < nonterminals.Length(); k++)
            {
                int lhs = nonterminals[k];

                Array<bool> rule_set(grammar -> rules.Length() - 1, false);
                for (int i = 0; i < grammar -> RulesOfLhs(lhs).Length(); i++)
                    rule_set[grammar -> RulesOfLhs(lhs)[i]] = true;

                int j = 0;
                for (; j < nonterminal_map[lhs].Length(); j++)
                {
                    int rule_no = nonterminal_map[lhs][j];
                    if (rule_no <= grammar -> num_rules && (! rule_set[rule_no]))
                        break;
                }
                if (j == nonterminal_map[lhs].Length())
                    is_list_nonterminal[lhs] = true;
                else if (option -> warnings)
                {
                    int rule_no = grammar -> RulesOfLhs(lhs)[0],
                               ...
                               Tuple<ClassnameElement> &classname)
{
    ...
    Array<RuleAllocationElement> rule_allocation_map(grammar -> rules.Length() - 1); // (grammar -> num_rules + 1);

    //
    // Map each rule into the set of action blocks that is associated with it.
    //
    Array< Tuple<ActionBlockElement> > rule_action_map(grammar -> rules.Length() - 1); //(grammar -> num_rules + 1);
    {
    ...
    //
    //
    TTC ttc(grammar -> lhs_rule, processed_rule_map);
    ...
    CompleteClassnameInfo(lca, ttc, typestring, processed_rule_map, classname_set, classname, rule_allocation_map);
    ...
    //
    Array< bool > user_specified_null_ast(grammar -> rules.Length() - 1, false); // (grammar -> num_rules + 1, false);
    {
        for (int rule_no = grammar -> FirstRule(); rule_no <= grammar -> LastRule(); rule_no++)
        {
            int source_index = grammar -> rules[rule_no].source_index,
    ...
    NTC ntc(user_specified_null_ast, grammar);
...
void Action::ProcessMacro(TextBuffer *buffer, const char *name, int rule_no)
{
    ...
    char *macroname = new char[length + 1];
        ...
        else if (! FindUndeclaredMacro(macroname, length))
        {
            ...
            msg.Put("The macro \"");
            msg.Put(macroname);
            msg.Put("\" must be defined when the \"automatic_ast\" option is in effect");
...
//
//
void Action::EmitMacroError(const char *filename, const char *start_cursor_location, const char *end_cursor_location, TextBuffer &msg)
{
...
//
//
void Action::EmitMacroWarning(const char *filename, const char *start_cursor_location, const char *end_cursor_location, TextBuffer &msg)
{
    Tuple<Token *> macro_token;
    GetCallingMacroLocations(macro_token);
...
Symbol *Action::FindClosestMatchForMacro(const char *filename, const char *cursor, const char *end_cursor, const char *start_cursor_location, const char *end_cursor_location)
{
    ...
    char *macro_name = new char[length + 1];
        ...
        {
            msg.Put("LPG will assume you meant \"");
            msg.Put(symbol -> Name());
            msg.Put("\"");
        }
            ...
            else if ((simple_macro = FindLocalMacro(cursor, end_cursor - cursor)) != NULL)
            {
                ...
                else if (simple_macro == rule_text_macro)
                {
                    ...
                    else
                    {
                        ...
                        if (rule_no > grammar -> num_rules) // Not a Prosthesis rule?
                        {
                            buffer -> Put(" (prosthesis)");
                        }
                        else
                        {
                            buffer -> PutChar(' ');
...
//
// The syntax for specifying a prosthesis used to be:
//
//     A ::= %ERROR
//
// We no longer accept this syntax.
//
//                            if (rule_no > grammar -> num_rules) // A Prosthesis rule?
//                            {
//                                buffer -> PutChar(' ');
//                                buffer -> PutChar(option -> escape);
//                                buffer -> Put("Error");
//                            }
                ...
                else if (simple_macro == identifier_macro)
                {
                    ...
                    TextBuffer msg;
                    msg.Put("No explicit user definition for $identfier - IDENTIFIER is assumed");
            ...
            else if ((macro = FindUserDefinedMacro(cursor, end_cursor - cursor)) != NULL)
            {
                ...
                {
                    TextBuffer msg;
                    msg.Put("Loop detected during the expansion of the macro \"");
                    msg.Put(macro -> Name());
                    msg.Put("\"");

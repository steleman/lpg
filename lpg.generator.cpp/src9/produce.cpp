...
Produce::Produce(Control *control_) : control(control_),
...
//
void Produce::ProcessAdvancedMaps(void)
{
    //
    ...
    useful.Resize(grammar -> num_terminals + 1, grammar -> num_symbols);
    useful.MemReset();
    for (int state_no = 1; state_no <= dfa -> num_states; state_no++)
    {
        ...
        for (int i = 0; i < go_to.Length(); i++)
        {
            ...
            {
                Node *q = dfa -> statset[state].kernel_items;
                int item_no = q -> value;
                if (q -> next != NULL)
                     rule_no = 0;
    ...
    assert (gd_index.Length() == dfa -> num_states + 1);
...
void Produce::process_scopes(void)
{
    ...
    for (int item_no = 1; item_no <= grammar -> num_items; item_no++)
    {
        ...
        if (grammar -> IsRecover(dot_symbol))
        {
            ...
            else
            {
                ...
                TextBuffer msg;
                msg.Put("This rule is not processed as a scope because the prefix preceding the recovery symbol \"");
                msg.Put(tok);
                msg.Put("\" is nullable");
    ...
    //
    {
        Array<BitSet> collection(dfa -> num_states + 1);
        {
            for (int i = 1; i <= dfa -> num_states; i++)
                collection[i].Initialize(dfa -> num_states + 1);
        }
        ...
        scope_state[0] = 0;
        Array<int> state_list(dfa -> num_states + 1, Util::OMEGA);
        int k = 0;
        for (int i = 1; i <= num_state_sets; i++)
        {
            if (start[i] > 0)
...
//
void Produce::ProcessScopesOutput(void)
{
    if (! option -> list)
        return;

...
void JavaTable::print_symbols(void)
{
    ...
    for (symbol = grammar -> FirstTerminal(); symbol <= grammar -> LastTerminal(); symbol++)
    {
        ...
        if (tok[0] == '\n' || tok[0] == option -> escape)
        {
            ...
            TextBuffer msg;
            msg.Put("Escaped symbol ");
            msg.Put(tok);
            msg.Put(" may be an invalid Java variable.");
        ...
        else if (strpbrk(tok, "!%^&*()-+={}[];:\"`~|\\,.<>/?\'") != NULL)
        {
            TextBuffer msg;
            msg.Put(tok);
            msg.Put(" is an invalid Java variable name.");
...
void JavaTable::print_exports(void)
{
    ...
    *tok = '\0';
    ...
    for (int i = 1; i <= grammar -> exported_symbols.Length(); i++)
    {
        ...
        tok = new char[variable_symbol -> NameLength() + 1];
        ...
        if (tok[0] == '\n' || tok[0] == option -> escape)
        {
            ...
            TextBuffer msg;
            msg.Put("Escaped exported symbol ");
            msg.Put(tok);
            msg.Put(" may be an invalid Java variable.");
...
void JavaTable::print_definitions(void)
{
    if (option -> serialize)
    {
        ...
        Serialize("scopes", "getScopes", (bool) (produce -> scope_prefix.Size() > 0));
    }

    print_definition("ERROR_SYMBOL", "getErrorSymbol", option -> error_maps ? grammar -> error_image : 0);
    print_definition("SCOPE_UBOUND", "getScopeUbound", option -> error_maps ? produce -> scope_prefix.Size() - 1 : 0);
    print_definition("SCOPE_SIZE", "getScopeSize", option -> error_maps ? produce -> scope_prefix.Size() : 0);
    print_definition("MAX_NAME_LENGTH", "getMaxNameLength", option -> error_maps ? max_name_length : 0);
    print_definition("NUM_STATES", "getNumStates", dfa -> num_states);
...
void JavaTable::print_serialized_tables(void)
{
    ...
    if (option -> error_maps)
    {
        ...
        ConditionalSerialize("", *array_info[PROSTHESIS_RULE_INDEX]);
    }
    else
    {
        ...
        assert(array_info[SCOPE_STATE] == NULL);
        assert(array_info[IN_SYMB] == NULL);
        assert(array_info[PROSTHESIS_RULE_INDEX] == NULL);
        ...
        ConditionalDeserialize("", SCOPE_STATE, I32);
        ConditionalDeserialize("", IN_SYMB, I32);
        ConditionalDeserialize("", PROSTHESIS_RULE_INDEX, I32);
    }
    des_buffer.Put("            }\n");
...
void JavaTable::print_source_tables(void)
{
    ...
    else
    {
        prs_buffer.Put("    public final int asb(int index) { return 0; }\n"
                       ...
                       "    public final int scopeState(int index) { return 0; }\n"
                       "    public final int inSymb(int index) { return 0; }\n"
                       "    public final int prosthesisRuleIndex(int index) { return 0; }\n"
...
//
//
void JavaTable::OutputTables(void)
{
    init_parser_files();

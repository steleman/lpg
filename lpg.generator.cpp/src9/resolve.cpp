...
// Constructor
//
Resolve::Resolve(Control *control_) : control(control_),
                                      ...
                                      visited(control_ -> dfa -> num_states),
                                      sources(control_ -> grammar -> num_rules, control_ -> dfa -> num_states)
                                                 
{
    ...
    // not LR(k) for any k.
    //
    cyclic.Resize(dfa -> num_states + 1);
    index_of.Resize(dfa -> num_states + 1);
...
//
// Starting with a complete item on which a shift-reduce conflict was detected, we look
// for an item that could have been the source of the conflict. I.e., a "source" item where
// the dot precedes a nonterminal from which the completed item can be derived and the First
// map of the suffix following the nonterminal in question (in the source item) contains the
// conflict symbol.
//
int Resolve::ShiftReduceConflictSourceItem(int state_no, int item_no, int conflict_symbol)
{
    ...
    if (lhs_symbol != grammar -> accept_image)
    {
        ...
        dfa -> Access(state_list, state_no, item_no);
        for (int k = 0; k < state_list.Length(); k++)
        {
            int access_state = state_list[k],
                i = dfa -> statset[access_state].go_to.Index(lhs_symbol);
            item_no = ShiftReduceConflictSourceTrace(access_state, i, conflict_symbol);
            if (item_no != 0) // found a source item
                break;
        }
    }
...
//
// Recursive trace function for ShiftReduceConflictSourceItem. See above. 
//
int Resolve::ShiftReduceConflictSourceTrace(int state_no, int goto_indx, int conflict_symbol)
{
    Dfa::GotoHeader &go_to = dfa -> statset[state_no].go_to;
    lalr_visited[go_to[goto_indx].Laptr()] = true;
    Tuple<int> state_list;
    int state = go_to[goto_indx].Action();
    for (Node *p = (state > 0 ? dfa -> statset[state].kernel_items
                              : base -> adequate_item[-state]);
         p != NULL;
         p = p -> next)
    {
        int item_no = p -> value - 1;
        if (base -> First(base -> item_table[item_no].suffix_index)[conflict_symbol]) // Conflict_symbol can be read in state?
            return item_no;
        else if (base -> First(base -> item_table[item_no].suffix_index)[grammar -> empty])
        {
            ...
            {
                int access_state = state_list[k];
                Dfa::GotoHeader &go_to = dfa -> statset[access_state].go_to;
                                 ...
                                 int conflict_symbol)
{
         ...
         p = p -> next)
    {
        ...
        else if (base -> First(base -> item_table[item].suffix_index)[grammar -> empty])
        {
            ...
            dfa -> Access(state_list, state_no, item);
            for (int k = 0; k < state_list.Length(); k++)
            {
                int access_state = state_list[k];
                Dfa::GotoHeader &go_to = dfa -> statset[access_state].go_to;
...
void Resolve::print_relevant_slr_items(int item_no, int conflict_symbol)
{
    ...
    (void) slr_trace(grammar -> rules[base -> item_table[item_no].rule_number].lhs,
                     conflict_symbol);
...
int Resolve::ComputeShiftPriority(int state_no, int action, int conflict_symbol)
{
    ...
    for (Node *p = (action > 0 ? dfa -> statset[action].kernel_items
...
//      option -> report.PutChar('\n');
        ...
        TextBuffer msg,
                   formatted_item;
        int rule_no_num = grammar -> RuleNumberOf(rule_no);
        dfa -> FormatItem(formatted_item, ShiftReduceConflictSourceItem(state_no, p -> item, symbol));

	//
	// Shift/reduce conflict in state 1 (in item: CompilationUnit ::= .PackageDeclarationopt ImportDeclarationsopt TypeDeclarationsopt)
        // on "@" with rule 543 (1st rule of PackageDeclarationopt)
	//
        msg.Put("Shift/reduce conflict in state ");
        msg.Put(state_no);
        msg.Put(" at item   ");
        msg.Put(formatted_item);
        msg.Put("   on token \"");
        msg.Put(temp);
        msg.Put("\" with rule ");
        msg.Put(rule_no);
        ...
        msg.Put(rule_no_num == 1
                             ...
                             : rule_no_num == 2
                                            ...
                                            : rule_no_num == 3
                                                           ...
                                                           : "th");
        msg.Put(" rule of ");
        msg.Put(grammar -> RetrieveString(grammar -> rules[rule_no].lhs));
        msg.Put(")");
    ...
    for (int j = 0; j < rr_conflicts.Length(); j++)
    {
        ...
        TextBuffer msg;
        int n_num = grammar -> RuleNumberOf(n);
        int rule_no_num = grammar -> RuleNumberOf(rule_no);

        msg.Put("Reduce/reduce conflict in state ");
        msg.Put(state_no);
        msg.Put(" on \"");
        msg.Put(temp);
        msg.Put("\" between rule ");
        msg.Put(n);
        msg.Put(" (");
        msg.Put(n_num);
        msg.Put(n_num == 1
                       ...
                       : n_num == 2
                                ...
                                : n_num == 3
                                         ...
                                         : "th");
        msg.Put(" rule of ");
        msg.Put(grammar -> RetrieveString(grammar -> rules[n].lhs));
        msg.Put(")");
        msg.Put(" and rule ");
        msg.Put(rule_no);
        msg.Put(" (");
        ...
        msg.Put(grammar -> RetrieveString(grammar -> rules[rule_no].lhs));
        msg.Put(")");
        msg.Put(" starting at line ");
        msg.Put(otherStartToken -> Line());
        msg.Put(", column ");
        msg.Put(otherStartToken -> Column());
                                               ...
                                               int symbol, int la_symbol)
{
    ...
    if (grammar -> IsTerminal(symbol))
        act = dfa -> Shift(state_no).Action(symbol);
    else
    {
        Dfa::GotoHeader &go_to = dfa -> statset[state_no].go_to;
        int i = go_to.Index(symbol);
    ...
    if (act > 0)
    {
        ...
        // configurations...
        //
        Dfa::ShiftHeader &sh = dfa -> Shift(act);
        ...
        if (! cyclic[act])
        {
            Dfa::GotoHeader &go_to = dfa -> statset[act].go_to;
            for (int i = 0; i < go_to.Length(); i++)
            {
    ...
    //
    Tuple<int>state_list;
    for (Node *item_ptr = (act > 0 ? dfa -> statset[act].kernel_items
        ...
        if (base -> item_table[item_no].symbol == grammar -> empty)
        {
            ...
            if (lhs_symbol != grammar -> accept_image && (! base -> rmpself[lhs_symbol]))
            {
                ...
                else
                {
                    ...
                    // with these sources.
                    //
                    dfa -> Access(state_list, q -> state_number, item_no);
                    for (int k = 0; k < state_list.Length(); k++)
                    {
                        int access_state = state_list[k];
                        Resolve::StackElement *new_configs,
...
void Resolve::next_la(Resolve::StackElement *stack, int symbol, BitSet &look_ahead)
{
    ...
    //
    int state_no = stack -> state_number,
        act = (grammar -> IsTerminal(symbol)? dfa -> Shift(state_no).Action(symbol)
                                            : dfa -> statset[state_no].go_to.Action(symbol));
...
int Resolve::state_to_resolve_conflicts(Resolve::SourcesElement &sources, int la_symbol, int level)
{
    ...
    //
    Resolve::SourcesElement new_sources(grammar -> num_rules, dfa -> num_states);
    ...
    //
    pda -> max_la_state++;
    assert(pda -> max_la_state == dfa -> statset.Length());

    state = dfa -> statset.NextIndex();

    dfa -> statset[state].kernel_items = NULL;
    dfa -> statset[state].complete_items = NULL;
    dfa -> statset[state].single_production_items = NULL;
    dfa -> statset[state].transition_symbol = la_symbol;
      ...
      Array<int> shift_action(grammar -> num_terminals + 1, Util::OMEGA);
        ...
        for (int i = 0; i < shift_list.Length(); i++)
        {
            ...
            if (action[symbol] -> value < 0)
                dfa -> num_shift_reduces++;
            else if (action[symbol] -> value <= dfa -> num_states)
                dfa -> num_shifts++;
            else // lookahead-shift
                dfa -> AddPredecessor(action[symbol] -> value, state);

            shift_action[symbol] = action[symbol] -> value;
        }

        dfa -> statset[state].shift_number = dfa -> UpdateShiftMaps(shift_list, shift_action);
    }
    else
    {
        dfa -> statset[state].shift_number = 0;
    }
                                                ...
                                                Array<int> &rule_count)
{
        ...
        for (int i = 0; i < grammar -> keywords.Length(); i++)
        {
            ...
            if (shift_action[keyword] == Util::OMEGA)
            {
                if (action[keyword].Length() == 0)
                {
                    ...
                    // in pda.cpp.
                    //
                    dfa -> statset[state_no].soft_keywords.Next() = keyword;
                }
                else
                {
                    ...
                    //
                    //
                    if (identifier_action > dfa -> num_states)
                    {
                        int offset = grammar -> num_rules; // the highest priority of a real action
                        for (int k = identifier_action - dfa -> num_states; dfa -> conflicts[k] != 0; k++)
                        {
                            int act = dfa -> conflicts[k];
                    ...
                    conf.SetConflictIndex(dfa -> MapConflict(cells));
    ...
    else if (identifier_action != Util::OMEGA) // (action[grammar -> identifier_image].Length() > 0)
    {
        ...
        for (int i = 0; i < shifted_keywords.Length(); i++)
        {
            ...
            {
                int offset = grammar -> num_rules; // the highest priority of a real action
                for (int k = identifier_action - grammar -> num_rules; dfa -> conflicts[k] != 0; k++)
                {
                    int act = dfa -> conflicts[k];
    ...
    if (symbol_action != Util::OMEGA)
    {
        ...
        if (action[symbol].Length() > 0)
        {
            ...
            if (option -> backtrack)
            {
                ...
                //
                symbol_action = conf.ConflictIndex() + dfa -> num_states;
            }
                                          ...
                                          Array< Tuple<int> > &action)
{
    ...
    if (action[symbol].Length() > 0)
    {
        ...
        if (option -> lalr_level > 1)
        {
            ...
            if (la_state != Util::OMEGA)
            {
                dfa -> AddPredecessor(la_state, state_no);
                action[symbol].Reset();
            }
        }
...

{
    ...
    if (action[symbol].Length() > 1 || first_rule != identifier_action)
    {
        ...
        {
            int offset = grammar -> num_rules; // the highest priority of a real action
            for (int i = identifier_action - grammar -> num_rules; dfa -> conflicts[i] != 0; i++)
            {
        ...
        if (option -> conflicts)
        {
            ...
            int rule_no2 = (identifier_action <= grammar -> num_rules
                                ? identifier_action
                                : dfa -> conflicts[identifier_action - grammar -> num_rules]);
...
{
    dfa -> statset.Resize(pda -> max_la_state + 1);
    ...
    for (int state_no = 1; state_no <= pda -> max_la_state; state_no++)
        shift_count[dfa -> statset[state_no].shift_number]++;
    ...
    //
    Array<Tuple <int> > new_shift_actions(dfa -> num_states + 1);
    Tuple<int> state_list;
    for (int la_state = dfa -> num_states + 1; la_state <= pda -> max_la_state; la_state++)
    {
        if (dfa -> statset[la_state].predecessors.Length() > 0) // unreachable states have no predecessors
        {
            assert(dfa -> statset[la_state].predecessors.Length() == 1); // there is exactly one predecessor
            int state_no = dfa -> statset[la_state].predecessors[0];
    ...
    for (int j = 0; j < state_list.Length(); j++)
    {
        ...
        //
        int shift_no = dfa -> statset[state_no].shift_number;
        ...
        for (int k = 0; k < new_shift_actions[state_no].Length(); k++)
        {
            int la_state = new_shift_actions[state_no][k],
                symbol = dfa -> statset[la_state].transition_symbol;
        ...
        if (shift_count[shift_no] > 1)
        {
             ...
             dfa -> statset[state_no].shift_number = dfa -> UpdateShiftMaps(shift_list, shift_action);
             shift_count[dfa -> statset[state_no].shift_number]++;
        }
        else dfa -> ResetShiftMap(shift_no, shift_list, shift_action);

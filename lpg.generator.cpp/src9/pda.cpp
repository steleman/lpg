...
// Constructor
//
Pda::Pda(Control *control_) : control(control_),
                              option(control_ -> option),
                              grammar(control_ -> grammar),
                              base(control_ -> base),
                              dfa(control_ -> dfa),
                              produce(control_ -> produce),
...
void Pda::TraceLalrPath(int state_no, int goto_index)
{
    ...
    Node *kernel_root = (state > 0 ? dfa -> statset[state].kernel_items : base -> adequate_item[-state]);
...
void Pda::ComputeRead(void)
{
    ...
    {
        read_set.Resize(dfa -> num_states + 1);
    ...
    //
    Tuple<int> state_list;
    la_base.Resize(dfa -> num_states + 1);
    ...
    for (int state_no = 1; state_no <= dfa -> num_states; state_no++)
    {
        for (Node *p = ((option -> lalr_level <= 1 && single_complete_item[state_no])
                        ? NULL
                        : dfa -> statset[state_no].complete_items);
             p != NULL;
             p = p -> next)
        {
            int item_no = p -> value,
                ...
                if (la_ptr != Util::OMEGA)   // Follow Look-ahead needed
                {
                    ...
                    if (state > 0)
                    {
                        ...
                        else
                        {
                            ...
                            q = dfa -> statset[state].kernel_items;
                        }
                    }
...
void Pda::LaTraverse(int state_no, int goto_index)
{
    ...
    Tuple<int> state_list;
    int state = go_to[goto_index].Action();
    for (Node *r = (state > 0 ? dfa -> statset[state].kernel_items : base -> adequate_item[-state]);
         r != NULL;
         r = r -> next)
    {  // loop over items [A -> x LHS_SYMBOL . y]
        ...
        if (base -> First(base -> item_table[item].suffix_index)[grammar -> empty])
        {
            ...
            dfa -> Access(state_list, state_no, item); // states where RULE was introduced through closure
            for (int k = 0; k < state_list.Length(); k++)
            {
                //
...
//
// ProcessPDA() constructs the REDUCE map and detects conflicts in the grammar.
...
void Pda::ProcessPDA(void)
{
    ...
    //
    max_la_state = dfa -> statset.Length() - 1;
    ...
    //
    Resolve *resolve = new Resolve(control);
    ...
    if (option -> lalr_level > 1)
    {
        ...
        for (int state_no = 1; (! not_lrk) && state_no <= dfa -> num_states; state_no++)
    ...
    if (option -> single_productions)
        conflict_symbols.Resize(dfa -> num_states + 1);
    ...
    //
    single_complete_item.Resize(dfa -> num_states + 1);
    {
        for (int state_no = 1; state_no <= dfa -> num_states; state_no++)
        {
            //
            ...
            //
            Node *item_ptr = dfa -> statset[state_no].kernel_items;
            ...
            if (highest_level == 0)
            {
                Node *p = dfa -> statset[state_no].complete_items;
                if (p != NULL)
                {
    ...
    for (int state_no = 1; state_no <= dfa -> num_states; state_no++)
    {
        ...
        Node *item_ptr = dfa -> statset[state_no].complete_items;
        if (item_ptr != NULL)
        {
            //
...
//                 }
        ...
        if (symbol_root != Util::NIL)
        {
            ...
            //
            int max_count = 0;
            for (Node *q = dfa -> statset[state_no].complete_items; q != NULL; q = q -> next)
            {
                int item_no = q -> value,
                    rule_no = base -> item_table[item_no].rule_number,
                    symbol = grammar -> rules[rule_no].lhs;
    ...
    //
    if (max_la_state > dfa -> num_states)
    {
        assert(resolve);
    ...
    else
    {
        ...
        if (num_reduce_reduce_conflicts > 0 || num_shift_reduce_conflicts > 0)
        {
            ...
            msg.Put("Grammar is not ");
            if (! option -> slr)
            {
                if (highest_level != Util::INFINITY)
                {
                    msg.Put("LALR(");
                    msg.Put(highest_level);
                    msg.Put(")")
...
;
            ...
            {
                msg.Put(num_reduce_reduce_conflicts);
                msg.Put(" reduce/reduce conflicts");
                if (num_shift_reduce_conflicts > 0)
                    msg.Put(" and ");
            }
            if (num_shift_reduce_conflicts > 0)
            {
                msg.Put(num_shift_reduce_conflicts);
                msg.Put(" shift/reduce conflicts");
            }
            msg.Put(".");
...
{
    assert(state_no > 0 && state_no <= dfa -> num_states);
    ...
    //
    strcpy(line, "( ");
    for (int k = 0; k < dfa -> statset[state_no].predecessors.Length(); k++)
    {// copy list of transition symbols into array
        int previous_state = dfa -> statset[state_no].predecessors[k];
    ...
    //
    Tuple<int> item_list;
    for (Node *p = dfa -> statset[state_no].kernel_items; p != NULL; p = p -> next)
    {
        int item_no = p -> value;
    ...
    //
    for (Node *r = dfa -> statset[state_no].single_production_items; r != NULL; r = r -> next)
    {
        int item_no = r -> value;
        if (! item_seen[item_no])
        {
    ...
    for (int i = 0; i < sh.Length(); i++)
    {
        ...
        while (next_state > dfa -> num_states && next_state < max_la_state)
        {
            Dfa::ShiftHeader &next_sh = dfa -> shift[dfa -> statset[next_state].shift_number];
        ...
        {
            q = NULL;
            for (int k = next_state - max_la_state; dfa -> conflicts[k] != 0; k++)
            {
                ...
                else if (act > grammar -> num_rules)
                {
                     ...
                     q = dfa -> statset[act].kernel_items;
                     if (q == NULL) // single production state?
                         q = dfa -> statset[act].complete_items;
                     break;
                }
            }
        }
        ...
        {
             q = dfa -> statset[next_state].kernel_items;
             if (q == NULL) // single production state?
                 q = dfa -> statset[next_state].complete_items;
        }
        for (; q != NULL; q = q -> next)
        {
            int item_no = q -> value - 1;
    ...
    for (int m = 0; m < go_to.Length(); m++)
    {
        ...
        if (go_to[m].Action() > 0)
        {
             q = dfa -> statset[go_to[m].Action()].kernel_items;
             if (q == NULL) // single production state?
                 q = dfa -> statset[go_to[m].Action()].complete_items;
        }
...
{
    assert(state_no > dfa -> num_states && state_no <= max_la_state);
    ...
    // Print the set of states that have transitions to STATE_NO.
    //
    if (dfa -> statset[state_no].predecessors.Length() == 0)
         fprintf(option -> syslis,"\n(Unreachable State)\n");
    else fprintf(option -> syslis,"\n(%d)\n", dfa -> statset[state_no].predecessors[0]);
...
void Pda::PrintShifts(Dfa::ShiftHeader &sh, int max_size)
{
    if (sh.Length() > 0)
    {
        ...
        for (int i = 0; i < sh.Length(); i++)
        {
            ...
            {
                for (int k = sh[i].Action() - max_la_state; dfa -> conflicts[k] != 0; k++)
                {
                    int act = dfa -> conflicts[k];
...
void Pda::ProcessConflictActions()
{
    ...
    for (int state_no = 1; state_no <= dfa -> num_states; state_no++)
    {
        ...
        Tuple<int> &soft_keywords = dfa -> statset[state_no].soft_keywords;
        if (conflict.Length() > 0 || soft_keywords.Length() > 0)
        {
            ...
            for (int k = 0; k < conflict.Length(); k++)
            {
                ...
                //
                for (int i = index; dfa -> conflicts[i] != 0; i++)
                {
...
//
// PrintAllStates prints all the states of the parser.
//
void Pda::ProcessPDAOutput(void)
{
    if (! option -> states)
        return;
    ...
    if (max_la_state > dfa -> num_states)
    {
        for (int state_no = dfa -> num_states + 1; state_no <= max_la_state; state_no++)
        {
            if (dfa -> statset[state_no].predecessors.Length() == 1) // a reachable lookahead state
            {
                Dfa::ShiftHeader &sh = dfa -> Shift(state_no);
                for (int j = 0; j < sh.Length(); j++)
                {
                    int action = sh[j].Action();
                    if (action > 0 && action <= dfa -> num_states) // a shift action
                        dfa -> AddPredecessor(action, state_no);
                }
            }
        }
    }
    ...
    for (int state_no = 1; state_no <= dfa -> num_states; state_no++) // iterate over the states
    {
        PrintShiftState(state_no);

        int max_size = MaximumSymbolNameLength(state_no);

        PrintShifts(dfa -> Shift(state_no), max_size);
        PrintGotos(dfa -> statset[state_no].go_to, max_size);
        PrintReductions(dfa -> statset[state_no].reduce, max_size);
    }
    ...
    if (max_la_state > dfa -> num_states)
    {
        ...
        for (int state_no = dfa -> num_states + 1; state_no <= max_la_state; state_no++)
        {
            ...
            if (dfa -> statset[state_no].predecessors.Length() != 0) // This state is reachable
            {
                int max_size = MaximumSymbolNameLength(state_no);
                PrintShifts(dfa -> Shift(state_no), max_size);
                PrintReductions(dfa -> statset[state_no].reduce, max_size);

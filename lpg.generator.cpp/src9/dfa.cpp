...
Dfa::Dfa(Control *control_) : control(control_),
...
//
// PrintItem takes as parameter a text buffer and an ITEM_NO. It formats and
// outputs the item in question to the buffer.
//
void Dfa::FormatItem(TextBuffer &buffer, int item_no)
{
    //
    ...
    buffer.Put(grammar -> rules[rule_no].IsArrowProduction() ? " -> " : " ::= ");
    for (int i = 0; i < base -> item_table[item_no].dot; i++)
    {
        int symbol = grammar -> rhs_sym[grammar -> rules[rule_no].rhs_index + i];
        grammar -> RestoreSymbol(buffer, grammar -> RetrieveString(symbol));
        buffer.PutChar(' ');
    }
    ...
    for (int k = grammar -> rules[rule_no].rhs_index +
...
//
// PrintItem takes as parameter an ITEM_NO which it formats and prints
//

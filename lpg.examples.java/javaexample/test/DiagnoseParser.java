...
public class DiagnoseParser implements ParseErrorCodes
{
	...
	SEGMENT_SIZE = prs.getSegmentSize();
    ...
    public void diagnose()
    {
        ...
        int error_token = parseForError(stateStack, stateStackTop, current_kind);
    ...
    public void diagnose(int error_token)
    {
        ...
        stateStackTop = 0;
        stateStack[stateStackTop] = START_STATE;
        parseUpToError(stateStack, stateStackTop, action, current_kind, error_token);
        ...
        do
        {
            ...
            if (act == ERROR_ACTION)
            {
                ...
                tempStackTop = stateStackTop;
                System.arraycopy(stateStack, 0, tempStack, 0, stateStackTop + 1);
                error_token = parseForError(tempStack, tempStackTop, current_kind);
                ...
                if (error_token != 0)
                {
                    ...
                    tempStackTop = stateStackTop;
                    System.arraycopy(stateStack, 0, tempStack, 0, stateStackTop + 1);
                    parseUpToError(tempStack, tempStackTop, action, current_kind, error_token);
                    tokStream.reset(next_token);
                }
    ...
    //
    // Parse the input starting with the initial stack and the next token in
    // the tokStream until either the parse completes successfully or
    // it encounters an error. If the parse was not succesful, we return
    // the farthest token on which an error was detected. Otherwise, we return
    // 0.
    //
    int parseForError(int stack[], int stack_top, int current_kind)
    {
        int error_token = 0;
        ...
        int curtok = tokStream.getPrevious(tokStream.peek()),
            act = tAction(stack[stack_top], current_kind);

        //
        // Allocate configuration stack.
        //
        IntTuple conflicts = new IntTuple(1 << 2);
        ConfigurationStack configuration_stack = new ConfigurationStack(START_STATE);
        ...
        for (;;)
        {
            if (act <= NUM_RULES)
            {
                ...
                {
                    stack_top -= (rhs(act) - 1);
                    act = ntAction(stack[stack_top], lhs(act));
                } while(act <= NUM_RULES);
            }
            ...
            else if (act == ERROR_ACTION)
            {
                ...
                if (current_kind != EOFT_SYMBOL)
                {
                    ...
                    {
                        stack_top = configuration.stack_top;
                        configuration.retrieveStack(stack);
            ...
            {
                if (configuration_stack.findConfiguration(stack, stack_top, curtok))
                    act = ERROR_ACTION;
                else
                {
                    conflicts.reset();
                    for (int k = act + 1; baseAction(k) != 0; k++)
                        conflicts.add(baseAction(k));
                    configuration_stack.push(stack, stack_top, conflicts, curtok, 0);
                    act = baseAction(act);
                }
                continue;
            }
    ...
    //
    // Given this configuration, parse up to error_token and store
    // all the actions executed in the "action" tuple.
    //
    void parseUpToError(int stack[], int stack_top, IntTuple action, int current_kind, int error_token)
    {
        //
        // Assume predecessor of next token and compute initial action
        //
        int curtok = tokStream.getPrevious(tokStream.peek());
        int act = tAction(stack[stack_top], current_kind);
        ...
        for (;;)
        {
            ...
            {
                if (curtok != error_token && current_kind != EOFT_SYMBOL)
                {
                    ConfigurationElement configuration = configuration_stack.pop();
                    if (configuration == null)
                        act = ERROR_ACTION;
            ...
            else if (act > ACCEPT_ACTION)
            {
                ...
                else
                {
                    ...
                    for (int k = act + 1; baseAction(k) != 0; k++)
                        conflicts.add(baseAction(k));
                    configuration_stack.push(stack, stack_top, conflicts, curtok, action.size());
                    act = baseAction(act);
                }
                continue;
            }
    ...
    //
    private int parseCheck(int stack[], int stack_top, int first_symbol, int buffer_position)
    {
        int max_pos,
            buffer_index,
            current_kind;
        ...
        for (;;)
        {
            ...
            else if (act > ACCEPT_ACTION)
            {
                ...
                else
                {
                    ...
                    for (int k = act + 1; baseAction(k) != 0; k++)
                        conflicts.add(baseAction(k));
                    configuration_stack.push(local_stack, local_stack_top, conflicts, buffer_index, 0);
                    act = baseAction(act);
                }
                continue;
            }
    ...
    //
    private void checkPrimaryDistance(PrimaryRepairInfo repair, int stck[], int stack_top)
    {
        //
    ...
    private void scopeTrial(PrimaryRepairInfo repair, int stack[], int stack_top)
    {
    ...
    private void scopeTrialCheck(PrimaryRepairInfo repair, int stack[], int stack_top, int indx)
    {
                ...
                if (act != ERROR_ACTION)
                {
                    ...
                    if (scopeRhs(k) == 0 && j < marked_pos) // match?
                    {
                        ...
                        if (scopeState(j) != 0) // state was found
                        {
                            ...
                            if (tokStream.getKind(buffer[repair.bufferPosition]) == EOFT_SYMBOL &&
    ...
    //
    private boolean secondaryCheck(int stack[], int stack_top, int buffer_position, int distance)
    {
    ...
    //
    private void misplacementRecovery(SecondaryRepairInfo repair, int stack[], int stack_top, int last_index, boolean stack_flag)
    {
        ...
        for (int top = stack_top - 1; top >= 0; top--)
        {
            ...
            int j = parseCheck(stack, top, tokStream.getKind(buffer[2]), 3);
            if (j == MAX_DISTANCE)
                j = last_index;
    ...
    //
    private void secondaryRecovery(SecondaryRepairInfo repair, int stack[], int stack_top, int last_index, boolean stack_flag)
    {
        int previous_loc = buffer[2],
            stack_deletions = 0;
        ...
        for (int top = stack_top; top >= 0 && repair.numDeletions >= stack_deletions; top--)
        {
            ...
            {
                int j = parseCheck(stack, top, tokStream.getKind(buffer[i]), i + 1);
                ...
                for (int l = nasi(stack[top]); l >= 0 && nasr(l) != 0; l++)
                {
                    ...
                    j = parseCheck(stack, top, symbol, i);
                    if (j == MAX_DISTANCE)
                        j = last_index;
    ...
    //
    private void reportError(int msg_code,
                             int name_index,
                             int left_token,
                             int right_token,
                             int scope_name_index)
    {
        ...
        String token_name = (name_index >= 0 ? name(name_index) : "");
	...
	if (msg_code == SCOPE_CODE)
	{
		token_name = "\"";
                ...
                {
                    token_name += name(scopeRhs(i));
                    if (scopeRhs(i+1) != 0) // any more symbols to print?
                         token_name += " ";
                }
                token_name += "\" for " + (scope_name_index != 0 ? name(scope_name_index) : "structure") + " ";
	}

	tokStream.reportError(msg_code, locationInfo, token_name + " " );
...
**************************************************************************************/
    ...
    //
    private void printPrimaryMessage(int msg_code,
                                     int name_index,
                                     int left_token_loc,
                                     int right_token_loc,
                                     int scope_name_index)
    {
        ...
        System.out.print(tokStream.getFileName()
                         + ':' + left_line_no  + ':' + left_column_no
                         + ':' + right_line_no + ':' + right_column_no
                         + ": ");
        ...
        {
            case ERROR_CODE:
                 System.out.print("Parsing terminated at this token");
                 break;
            case BEFORE_CODE:
                 System.out.print(token_name + " inserted before this token");
                 break;
            case INSERTION_CODE:
                 System.out.print(token_name + " expected after this token");
                 break;
            case DELETION_CODE:
                 System.out.print(left_token_loc == right_token_loc
                                      ? "Unexpected symbol ignored"
                                      : "Unexpected symbols ignored");
                 break;
            case INVALID_CODE:
                 System.out.print(token_name.length() == 0
                                      ? "Unexpected input discarded"
                                      : "Invalid " + token_name);
                 break;
            case SUBSTITUTION_CODE:
                 System.out.print(token_name + " expected instead of this token");
                 break;
            case SCOPE_CODE:
                 System.out.print('"');
                 for (int i = scopeSuffix(- (int) name_index); scopeRhs(i) != 0; i++)
                 {
                     System.out.print(name(scopeRhs(i)));
                     if (scopeRhs(i+1) != 0) // any more symbols to print?
                         System.out.print(' ');
                 }
                 System.out.print('"' + " inserted to complete scope");

                 //
                 // TODO: This should not be an option
                 //
                 if (scope_name_index != 0)
                      System.out.print(name(scope_name_index));
                 else System.out.print("structure");
                 break;
            case MERGE_CODE:
                 System.out.print("symbols merged to form " + token_name);
                 break;
            case EOF_CODE:
                 System.out.print(token_name + " reached after this token");
                 break;
            default:
                 if (msg_code == MISPLACED_CODE)
                     System.out.print("misplaced construct(s)");
                 else if (token_name.length() == 0)
                     System.out.print("unexpected input discarded");
                 else
                     System.out.print(token_name + " expected instead");
                 break;
        }
    ...
    //
    private void printSecondaryMessage(int msg_code,
                                       int name_index,
                                       int left_token_loc,
                                       int right_token_loc,
                                       int scope_name_index)
    {
        ...
        switch(msg_code)
        {
            ...
            case SCOPE_CODE:
                ...
                System.out.print('"' + " inserted to complete ");
            ...
            case MERGE_CODE:
                ...
                break;
            default:
                System.out.print(msg_code == DELETION_CODE || token_name.length() == 0
                                    ? "Unexpected input discarded"
                                    : token_name + " expected instead");
        }
    ...
    public int tAction(int act, int sym)
    {
        ...
        if (act > prs.getLaStateOffset())
        {
            int next_token = tokStream.peek();
            act = prs.lookAhead(act - prs.getLaStateOffset(), tokStream.getKind(next_token));
            while(act > prs.getLaStateOffset())
            {

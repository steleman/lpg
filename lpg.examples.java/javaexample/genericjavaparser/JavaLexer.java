...
public class JavaLexer extends LpgLexStream implements JavaParsersym, JavaLexersym, RuleAction
{
    ...
    private static void compareFiles(String old_file, String new_file)
    {
        try
        {
            JavaLexer old_lexer, new_lexer;
            if (old_file.equals(""))
            {
                char [] input_chars = new char[0];
                old_lexer = new JavaLexer(input_chars, "null_file");
            }
            else old_lexer = new JavaLexer(old_file);

            if (new_file.equals(""))
            {
                char [] input_chars = new char[0];
                new_lexer = new JavaLexer(input_chars, "null_file");
            }
            else new_lexer = new JavaLexer(new_file);

...
%options template=Utf8LexerTemplateF.gi
%options filter=JavascriptKWLexerUtf8.gi

%Define
    --
    -- Definition of macro used in the included file LexerBasicMapF.gi
    --
    $kw_lexer_class /.$JavascriptKWLexerUtf8./
    ...
    /.
        ...
        public void makeToken(int startLoc, int endLoc, int kind)
        {
            ...
            if (parser.parse(sym, 0) == ERROR_ACTION)
            {
                ...
                if (getILexStream().getLineNumberOfCharAt(startLoc) > previous_token.getLine())
                {
        ...
        {
            Utf8LexStream utf8LexStream = (Utf8LexStream) getILexStream();
            ...
            byte [] input_bytes = utf8LexStream.getInputBytes();
            if (input_bytes == null)
            ...
            for (int byte_index = 0; 
                 byte_index < input_bytes.length;
                 byte_index = scanner.getLastToken() + 1)
            {
                if (byte_index + 1 < input_bytes.length &&
                    input_bytes[byte_index] == '/' &&
                    input_bytes[byte_index + 1] != '*' &&
                    input_bytes[byte_index + 1] != '/')
                {
                    sym[0] = (input_bytes[byte_index + 1] == '='
                                   ? $_SLASH_EQUAL
                                   : $_SLASH);
                    if (parser.parse(sym, 0) != ERROR_ACTION)
                    {
                        prsStream.makeToken(byte_index,
                                            (sym[0] == $_SLASH ? byte_index : byte_index + 1),
                                            sym[0]);
                        byte_index += (sym[0] == $_SLASH ? 1 : 2);
                    }
                }

                scanner.scanNextToken(byte_index);
            }

            int last_index = input_bytes.length - 1;
...
%Headers
    /.
        ...
        public $action_type(String filename) throws java.io.IOException
        {
            ...
            this.kwLexer = new $kw_lexer_class(((Utf8LexStream) getILexStream()).getInputBytes(), $_IDENTIFIER);
        }

        public void initialize(byte [] content, String filename)
        {
            Utf8LexStream utf8LexStream = (Utf8LexStream) getILexStream();
            utf8LexStream.initialize(content, filename);
        ...
        final void makeComment(int kind)
        {
            int startOffset = getLeftSpan(),
                endOffset = getRightSpan();
            getILexStream().getPrsStream().makeAdjunct(startOffset, endOffset, kind);
        }
        ...
        //
        //
        static class $super_stream_class extends Utf8LpgLexStream
        {
            //
            ...
            {
                return (i >= getStreamLength() ? $sym_type.$prefix$EOF : tokenKind[getUnicodeValue(i)]);
            }
            ...
            public $super_stream_class(byte[] input_bytes, String filename, int tab)
            {
                super(input_bytes, filename, tab);
            }
    
            public $super_stream_class(byte[] input_bytes, String filename)
            {
                this(input_bytes, filename, 1);
            }
        }
    ./
%End
